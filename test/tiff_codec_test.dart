...
void main() {
  group('TIFF', () {
    test('round-trips uncompressed and PackBits RGBA exactly', () async {
      ...
      expect((await decodeTiff(uncompressed)).bytes, source.bytes);
      expect((await decodeTiff(packBits)).bytes, source.bytes);
    });

    test('decodes an independently encoded LZW RGBA fixture', () async {
      final Image decoded = await decodeTiff(base64Decode(_lzwFixture));
    ...
    test('applies orientation while decoding', () async {
      ...
      final Image decoded = await decodeTiff(encoded);
    ...
    test('automatic dispatch decodes TIFF', () async {
      final Image source = _testImage();

      final Image decoded = await decodeImage(encodeTiff(source));
    ...
    test('enforces pixel limits and rejects truncation', () async {
      ...
      await expectLater(decodeTiff(encoded, maxPixels: 1), throwsA(isA<ImageCodecException>()));
      await expectLater(decodeTiff(Uint8List.sublistView(encoded, 0, 20)), throwsA(isA<ImageCodecException>()));
    });
  });
}
