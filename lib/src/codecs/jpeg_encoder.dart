// Portions derived from package:image 4.9.2 under the MIT license.
// ignore_for_file: type=lint

import 'dart:typed_data';

import 'package:imcodec/src/codecs/encoder.dart';
import 'package:imcodec/src/codecs/jpeg_marker.dart';
...
class JpegEncoder extends Encoder {
  ...
  void _calculateYUV(
    ...
    for (var pos = 0; pos < 64; pos++) {
      ...
      final int offset = (yy * width + xx) * 4;
      final int alpha = image.bytes[offset + 3];
      final int inverseAlpha = 255 - alpha;
      final int r = (image.bytes[offset] * alpha + 255 * inverseAlpha + 127) ~/ 255;
      final int g = (image.bytes[offset + 1] * alpha + 255 * inverseAlpha + 127) ~/ 255;
      final int b = (image.bytes[offset + 2] * alpha + 255 * inverseAlpha + 127) ~/ 255;

      // calculate YUV values
      ydu[pos] = ((_rgbYuvTable[r] + _rgbYuvTable[(g + 256)] + _rgbYuvTable[(b + 512)]) >> 16) - 128.0;
      udu[pos] = ((_rgbYuvTable[(r + 768)] + _rgbYuvTable[(g + 1024)] + _rgbYuvTable[(b + 1280)]) >> 16) - 128.0;
      vdu[pos] = ((_rgbYuvTable[(r + 1280)] + _rgbYuvTable[(g + 1536)] + _rgbYuvTable[(b + 1792)]) >> 16) - 128.0;
    }
  }
  ...
  void _initQuantTables(int sf) {
    ...
    for (var row = 0; row < 8; row++) {
      for (var col = 0; col < 8; col++) {
        ...
        _fdtblUv[k] = 1.0 / (_uvTable[_zigzag[k]] * aasf[row] * aasf[col] * 8.0);
        k++;
      }
    }
  }
  ...
  void _initHuffmanTable() {
    ...
    _yacHuffman = _computeHuffmanTable(stdAcLuminanceNrCodes, stdAcLuminanceValues);
  ...
  List<int?> _fDCTQuant(List<double> data, List<double> fdtbl) {
    ...
    for (var i = 0; i < 64; ++i) {
      ...
      _outputfDCTQuant[i] = (fDCTQuant > 0.0) ? ((fDCTQuant + 0.5).toInt()) : ((fDCTQuant - 0.5).toInt());
    }
