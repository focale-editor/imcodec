...
class WebPEncoder extends Encoder {
  ...
  Uint8List _encodeVP8L(Image image, int width, int height) {
    ...
    final header = (width - 1) | ((height - 1) << 14) | ((hasAlpha ? 1 : 0) << 28);
    out
      ..writeByte(0x2f)
    ...
    for (var i = 0; i < numPixels; i++) {
      ...
      r[i] = image.bytes[offset];
      g[i] = image.bytes[offset + 1];
      b[i] = image.bytes[offset + 2];
      a[i] = image.bytes[offset + 3];
    }
    ...
    final predModes = _selectPredictorModes(r, g, b, a, width, height, predBlockW, predBlockH, predBlockSize);
    ...
    while (j < numPixels) {
      ...
      if (j > 0 && candidates != null) {
        for (var ci = candidates.length - 1; ci >= 0; ci--) {
          ...
          var len = 1;
          while (len < maxMatchLen && j + len < numPixels && g[j + len] == g[c + len] && r[j + len] == r[c + len] && b[j + len] == b[c + len] && a[j + len] == a[c + len]) {
            len++;
          }
            ...
            for (var x = x0; x < x1; x++) {
              ...
              if (y == 0 && x == 0) {
                ...
                switch (m) {
                  ...
                  default: // 11: select
                    final tli = ti - 1;
                    final sl = (r[li] - r[tli]).abs() + (g[li] - g[tli]).abs() + (b[li] - b[tli]).abs();
                    final st = (r[ti] - r[tli]).abs() + (g[ti] - g[tli]).abs() + (b[ti] - b[tli]).abs();
                    if (sl <= st) {
                      pR = r[ti];
                      pG = g[ti];
                      pB = b[ti];
        ...
        if (y == 0 && x == 0) {
          ...
          switch (mode) {
            ...
            default: // 11: select(top, left, topLeft)
              final tli = ti - 1;
              final sl = (origR[li] - origR[tli]).abs() + (origG[li] - origG[tli]).abs() + (origB[li] - origB[tli]).abs() + (origA[li] - origA[tli]).abs();
              final st = (origR[ti] - origR[tli]).abs() + (origG[ti] - origG[tli]).abs() + (origB[ti] - origB[tli]).abs() + (origA[ti] - origA[tli]).abs();
              if (sl <= st) {
                pR = origR[ti];
                pG = origG[ti];
                pB = origB[ti];
  ...
  List<int> _buildHuffmanCodeLengths(
    ...
    for (var countMin = 1; ; countMin *= 2) {
      ...
      final pq = List<int>.generate(syms.length, (k) => k)..sort((x, y) => nodeFreq[x].compareTo(nodeFreq[y]));
