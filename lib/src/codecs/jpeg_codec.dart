...
final class JpegEncoder extends Encoder {
  ...
  List<List<int>?>? _luminanceDcHuffmanTable;

  /// Huffman lookup table for chrominance DC coefficients.
  List<List<int>?>? _chrominanceDcHuffmanTable;

  /// Huffman lookup table for luminance AC coefficients.
  late List<List<int>?> _luminanceAcHuffmanTable;

  /// Huffman lookup table for chrominance AC coefficients.
  late List<List<int>?> _chrominanceAcHuffmanTable;

  /// Signed coefficient bit patterns indexed around zero.
  final List<List<int>?> _coefficientBitCodes = List<List<int>?>.filled(65535, null);

  /// Signed coefficient category sizes indexed around zero.
  final List<int?> _coefficientCategories = List<int?>.filled(65535, null);
  ...
  /// Changes the compression quality for later encodes.
  ...
  Uint8List encode(
    ...
    // Add JPEG headers
    _writeMarker(output, _JpegMarker.startOfImage);
    ...
    if (_bytePos >= 0) {
      final List<int> fillBits = [(1 << (_bytePos + 1)) - 1, _bytePos + 1];
      _writeBits(output, fillBits);
    }

    _writeMarker(output, _JpegMarker.endOfImage);
  ...
  void _initQuantTables(int scaleFactor) {
    ...
    for (int row = 0; row < 8; row++) {
      for (int col = 0; col < 8; col++) {
        _luminanceTransformFactors[k] = 1.0 / (_yTable[_zigzag[k]] * aasf[row] * aasf[col] * 8.0);
        _chrominanceTransformFactors[k] = 1.0 / (_uvTable[_zigzag[k]] * aasf[row] * aasf[col] * 8.0);
        k++;
      }
    }
  }
  ...
  List<List<int>?> _computeHuffmanTable(List<int> codeCounts, List<int> standardValues) {
    int code = 0;
    int tablePosition = 0;
    final List<List<int>?> huffmanTable = <List<int>?>[<int>[]];
    for (int k = 1; k <= 16; k++) {
      for (int j = 1; j <= codeCounts[k]; j++) {
        final int index = standardValues[tablePosition];
        if (huffmanTable.length <= index) {
          huffmanTable.length = index + 1;
        }
        huffmanTable[index] = [code, k];
        tablePosition++;
        code++;
      }
      code *= 2;
  ...
  void _initHuffmanTable() {
    _luminanceDcHuffmanTable = _computeHuffmanTable(_standardLuminanceDcCodeCounts, _standardLuminanceDcValues);
    _chrominanceDcHuffmanTable = _computeHuffmanTable(_standardChrominanceDcCodeCounts, _standardChrominanceDcValues);
    _luminanceAcHuffmanTable = _computeHuffmanTable(_standardLuminanceAcCodeCounts, _standardLuminanceAcValues);
    _chrominanceAcHuffmanTable = _computeHuffmanTable(_standardChrominanceAcCodeCounts, _standardChrominanceAcValues);
  }
  ...
  void _initCategoryNumber() {
    ...
    for (int cat = 1; cat <= 15; cat++) {
      // Positive numbers
      for (int nr = lowerBound; nr < upperBound; nr++) {
        _coefficientCategories[32767 + nr] = cat;
        _coefficientBitCodes[32767 + nr] = [nr, cat];
      }
      // Negative numbers
      for (int nrNeg = -(upperBound - 1); nrNeg <= -lowerBound; nrNeg++) {
        _coefficientCategories[32767 + nrNeg] = cat;
        _coefficientBitCodes[32767 + nrNeg] = [upperBound - 1 + nrNeg, cat];
      }
      lowerBound <<= 1;
      upperBound <<= 1;
    }
  ...
  void _writeApplicationSegment(OutputBuffer out) {
    _writeMarker(out, _JpegMarker.applicationSegment);
    out
  ...
  void _writeStartOfFrameSegment(OutputBuffer out, int width, int height, JpegChroma chroma) {
    _writeMarker(out, _JpegMarker.startOfFrame);
    out
  ...
  void _writeQuantizationTables(OutputBuffer out) {
    _writeMarker(out, _JpegMarker.defineQuantizationTables);
    out
  ...
  void _writeHuffmanTables(OutputBuffer out) {
    _writeMarker(out, _JpegMarker.defineHuffmanTables);
    out
  ...
  void _writeStartOfScanSegment(OutputBuffer out) {
    _writeMarker(out, _JpegMarker.startOfScan);
    out
  ...
  int _encodeDataUnit(
    ...
    int dc,
    List<List<int>?>? dcHuffmanTable,
    List<List<int>?> acHuffmanTable,
  ) {
    final List<int>? endOfBlockCode = acHuffmanTable[0x00];
    final List<int>? sixteenZeroesCode = acHuffmanTable[0xf0];
    int pos;
    final List<int?> quantizedCoefficients = _transformAndQuantize(coefficients, transformFactors);

    // ZigZag reorder
    for (int j = 0; j < 64; ++j) {
      _zigzagCoefficients[_zigzag[j]] = quantizedCoefficients[j];
    }

    final int diff = _zigzagCoefficients[0]! - dc;
    final int currentDc = _zigzagCoefficients[0]!;
    // Encode dc
    if (diff == 0) {
      _writeBits(out, dcHuffmanTable![0]!); // diff might be 0
    } else {
      pos = 32767 + diff;
      _writeBits(out, dcHuffmanTable![_coefficientCategories[pos]!]!);
      _writeBits(out, _coefficientBitCodes[pos]!);
    }
    ...
    while (i <= lastNonZeroPosition) {
      ...
      if (zeroCount >= 16) {
        ...
        for (int markerIndex = 1; markerIndex <= runCount; ++markerIndex) {
          _writeBits(out, sixteenZeroesCode!);
        }
        zeroCount = zeroCount & 0xF;
      }
      pos = 32767 + _zigzagCoefficients[i]!;
      _writeBits(out, acHuffmanTable[(zeroCount << 4) + _coefficientCategories[pos]!]!);
      _writeBits(out, _coefficientBitCodes[pos]!);
      i++;
    }
  ...
  void _writeBits(OutputBuffer out, List<int> bits) {
    final int value = bits[0];
    int bitPosition = bits[1] - 1;
    while (bitPosition >= 0) {
...
abstract final class _JpegMarker {
  /// Start of frame, baseline discrete cosine transform.
  static const int startOfFrame = 0xc0;

  /// Define Huffman tables.
  static const int defineHuffmanTables = 0xc4;
  ...
  static const int defineQuantizationTables = 0xdb;

  /// JFIF application marker.
  static const int applicationSegment = 0xe0;
}
