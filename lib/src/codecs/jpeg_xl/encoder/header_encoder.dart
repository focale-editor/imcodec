...
/// Describes image dimensions and pixel format for an encoding operation.
final class JpegXlEncodeSetup {
  ...
  /// Number of significant bits in each sample.
  final int bitsPerSample;

  /// Whether the color data contains one grayscale plane.
  final bool grayscale;

  /// Whether an alpha plane follows the color planes.
  final bool hasAlpha;

  /// Creates and validates static encoding settings.
  JpegXlEncodeSetup({required this.width, required this.height, required this.bitsPerSample, required this.grayscale, required this.hasAlpha}) {
    if (width <= 0 || height <= 0) {
  ...
  /// Number of color planes before extra channels.
  int get colorChannels => grayscale ? 1 : 3;

  /// Total number of encoded planes.
  int get channelCount => colorChannels + (hasAlpha ? 1 : 0);
}

/// Writes one dimension using the compact or general JPEG XL representation.
...
/// regardless.
void writeImageHeader(BitWriter w, JpegXlEncodeSetup s, {bool xybEncoded = false}) {
  w.writeBits(0xFF, 8);
  w.writeBits(0x0A, 8);
  // SizeHeader.
  final bool div8 = s.width % 8 == 0 && s.height % 8 == 0 && s.width <= 256 && s.height <= 256;
  w.writeBool(div8);
...
void writeFrameHeader(BitWriter w, JpegXlEncodeSetup s, {int groupSizeShift = 1}) {
  w.writeBool(false); // all_default
