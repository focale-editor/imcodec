...
JpegXlDecodedImage buildDcImage(Frame frame, ImageHeader header, Uint8List? iccProfile, {bool isPreview = true}) {
  ...
  final int lowFrequencyHeight = padded.height >> 3;
  final int lowFrequencyWidth = padded.width >> 3;
  ...
  for (var c = 0; c < 3; c++) {
    final int sy = frame.header.jpegVerticalUpsamplingShift[c];
    final int sx = frame.header.jpegHorizontalUpsamplingShift[c];
    final List<Float32List> rows = floatMatrix(lowFrequencyHeight >> sy, lowFrequencyWidth >> sx);
    for (var g = 0; g < frame.lowFrequencyGroupCount; g++) {
      final LowFrequencyGroup? lowFrequencyGroup = frame.lowFrequencyGroups[g];
      if (lowFrequencyGroup == null || lowFrequencyGroup.lowFrequencyCoefficients == null) {
        throw const JpegXlInvalidBitstreamException(message: 'incomplete DC sections for preview');
      }
      final List<Float32List> src = lowFrequencyGroup.lowFrequencyCoefficients!.dequantizedLowFrequencyCoefficientsAt(c);
      final ({int x, int y}) pos = frame.lowFrequencyGroupLocation(g);
    ...
    if (sy != 0 || sx != 0) {
      final List<Float32List> full = floatMatrix(lowFrequencyHeight, lowFrequencyWidth);
      for (var y = 0; y < lowFrequencyHeight; y++) {
        final Float32List srcRow = rows[y >> sy];
        final Float32List dst = full[y];
        for (var x = 0; x < lowFrequencyWidth; x++) {
          dst[x] = srcRow[x >> sx];
        }
      }
      planes.add(full);
  ...
  return buildDcImageFromRows(planes, lowFrequencyHeight, lowFrequencyWidth, frame.header.usesYcbcr, header, iccProfile, isPreview: isPreview);
}
...
JpegXlDecodedImage buildDcImageFromRows(
  List<List<Float32List>> planes,
  int lowFrequencyHeight,
  int lowFrequencyWidth,
  bool usesYcbcr,
  ImageHeader header,
  ...
  if (header.xybEncoded) {
    final TransferFunction transferFunction = TransferFunction.forTransfer(header.colorEncoding.transferFunction);
    for (var c = 0; c < 3; c++) {
