...
final class JpegXlEncoder extends RasterEncoder with ParallelRasterEncoder {
  ...
  Uint8List encodeLossless(Uint8List pixels, {required int width, required int height, bool grayscale = false, bool hasAlpha = false, JpegXlEffort effort = JpegXlEffort.balanced}) {
  ...
  (JpegXlEncodeSetup, List<Int32List>) _losslessPlanes(Uint8List pixels, {required int width, required int height, required bool grayscale, required bool hasAlpha}) {
  ...
  Future<Uint8List> encodeLosslessWith(
  ...
  Uint8List encodeLossless16(Uint16List pixels, {required int width, required int height, bool grayscale = false, bool hasAlpha = false, JpegXlEffort effort = JpegXlEffort.balanced}) {
  ...
  Uint8List encodeLossy(Uint8List rgbPixels, {required int width, required int height, double distance = 1.0, VarDctConfiguration? config}) => encodeLossyVarDct(
    rgbPixels,
    width: width,
    height: height,
...
ModularResidualResult _runModularResidualJob(ModularResidualJob job) {
...
///
/// Top level and free of shared state, like [_runModularResidualJob].
...
const _paletteMaxColorsGray = 256;
...
Uint8List _encodeModular(JpegXlEncodeSetup setup, List<Int32List> planes, JpegXlEffort effort) {
  if (setup.grayscale) {
    final List<int>? palette = _detectPaletteGray(planes[0], _paletteMaxColorsGray);
  ...
  final List<int>? palette = _detectPalette(planes, _paletteMaxColors);
  if (palette == null) {
...
_ModularSteps _modularSteps(JpegXlEncodeSetup setup, List<Int32List> inputPlanes, List<int>? palette, bool applyRct, JpegXlEffort effort, {int paletteChannels = 3}) {
  ...
  bool refinesBoth(_Prep gradPrep, _Prep wpPrep) {
    final double gBits = gradPrep.tree.trainingBits;
    final double wBits = wpPrep.tree.trainingBits;
    return effort.refinesBothPredictors && gBits * _predictorMargin >= wBits && wBits * _predictorMargin >= gBits;
  }
...
Uint8List _encodeModularCore(JpegXlEncodeSetup setup, List<Int32List> inputPlanes, List<int>? palette, bool applyRct, JpegXlEffort effort, {int paletteChannels = 3}) {
  ...
  _Prep prep(bool useWp) {
    ...
    final _GatheredResiduals gathered = steps.gatherResiduals(
      useWp,
      [for (final ModularResidualJob job in pass.jobs) _runModularResidualJob(job)],
      pass.trainProps,
      pass.trainTokens,
    );
...
Future<Uint8List> _encodeModularCoreWith(
  ...
  if (!effort.triesBothPredictors) {
    final List<ModularResidualResult> results = await runner<ModularResidualJob, ModularResidualResult>(gradPass.jobs, _runModularResidualJob);
