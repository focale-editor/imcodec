...
import 'package:imcodec/src/codecs/jpeg_xl/util/image_buffer.dart' show floatMatrix;
...
/// Represents Dct params data used while processing JPEG XL images.
///
final class DctParams {
  /// Stores the dct param value used while processing JPEG XL data.
  ///
  final List<List<double>>? dctParam;

  /// Stores the param value used while processing JPEG XL data.
  ///
  final List<List<double>>? param;

  /// Stores the params4x4 value used while processing JPEG XL data.
  ///
  ...
  /// Stores the denominator value used while processing JPEG XL data.
  ///
  final double denominator;

  /// Creates Dct params data for JPEG XL processing.
  ///
  const DctParams({
    required this.dctParam,
    required this.param,
    required this.mode,
    this.params4x4,
    this.denominator = 1,
...
/// Stores the afv freqs state used internally by the JPEG XL codec.
///
...
/// Stores the seq a state used internally by the JPEG XL codec.
///
const _seqA = <double>[
...
/// Stores the seq b state used internally by the JPEG XL codec.
///
const _seqB = <double>[
...
/// Stores the seq c state used internally by the JPEG XL codec.
///
const _seqC = <double>[-1.2, -1.2, -0.8, -0.7, -0.7, -0.4, -0.5];

/// Processes the prepend data used by the JPEG XL codec.
///
List<double> _prepend(double a, List<double> rest) => [a, ...rest];

/// Stores the dct4x4 params state used internally by the JPEG XL codec.
///
...
/// Stores the dct4x8 params state used internally by the JPEG XL codec.
///
...
/// Stores the default dct params value used while processing JPEG XL data.
///
final List<DctParams> defaultDctParams = [
  ...
  /// Processes the prepend data used by the JPEG XL codec.
  ///
  DctParams(dctParam: [_prepend(23966.1665298448605, _seqA), _prepend(8380.19148390090414, _seqB), _prepend(4493.02378009847706, _seqC)], param: null, mode: TransformMode.dct),

  /// Processes the prepend data used by the JPEG XL codec.
  ///
  DctParams(dctParam: [_prepend(15358.89804933239925, _seqA), _prepend(5597.360516150652990, _seqB), _prepend(2919.961618960011210, _seqC)], param: null, mode: TransformMode.dct),

  /// Processes the prepend data used by the JPEG XL codec.
  ///
  DctParams(dctParam: [_prepend(47932.3330596897210, _seqA), _prepend(16760.38296780180828, _seqB), _prepend(8986.04756019695412, _seqC)], param: null, mode: TransformMode.dct),

  /// Processes the prepend data used by the JPEG XL codec.
  ///
  DctParams(dctParam: [_prepend(30717.796098664792, _seqA), _prepend(11194.72103230130598, _seqB), _prepend(5839.92323792002242, _seqC)], param: null, mode: TransformMode.dct),

  /// Processes the prepend data used by the JPEG XL codec.
  ///
  DctParams(dctParam: [_prepend(95864.6661193794420, _seqA), _prepend(33520.76593560361656, _seqB), _prepend(17972.09512039390824, _seqC)], param: null, mode: TransformMode.dct),
  ...
  /// Processes the prepend data used by the JPEG XL codec.
  ///
  DctParams(dctParam: [_prepend(61435.5921973295970, _seqA), _prepend(22389.44206460261196, _seqB), _prepend(11679.84647584004484, _seqC)], param: null, mode: TransformMode.dct),
];

/// Processes the quant mult data used by the JPEG XL codec.
///
double _quantMult(double v) => v >= 0 ? 1.0 + v : 1.0 / (1.0 - v);

/// Processes the interpolate data used by the JPEG XL codec.
///
double _interpolate(double scaledPos, List<double> bands) {
  ...
  final int scaledIndex = scaledPos.truncate();
  final double fracIndex = scaledPos - scaledIndex;
  if (scaledIndex + 1 > len) {
    return bands[len];
  }
  final double a = bands[scaledIndex];
  final double b = bands[scaledIndex + 1];
...
List<Float32List> getDCTQuantWeights(int height, int width, List<double> params) {
...
/// `rawWeightByType`).
List<Float32List> getDct4x4QuantWeights(List<double> dctParam, List<double> overrides) {
  final List<Float32List> target = floatMatrix(8, 8);
  final List<Float32List> w = getDCTQuantWeights(4, 4, dctParam);
  for (var y = 0; y < 8; y++) {
    for (var x = 0; x < 8; x++) {
...
List<Float32List> getDct4x8QuantWeights(List<double> dctParam, double override) {
  final List<Float32List> target = floatMatrix(8, 8);
  final List<Float32List> w = getDCTQuantWeights(4, 8, dctParam);
  for (var y = 0; y < 8; y++) {
    for (var x = 0; x < 8; x++) {
      target[y][x] = w[y ~/ 2][x];
    }
...
List<Float32List> getAFVQuantWeights(List<double> dctParam, List<double> params4x4, List<double> param) {
  final List<Float32List> weights4x8 = getDCTQuantWeights(4, 8, dctParam);
  final List<Float32List> weights4x4 = getDCTQuantWeights(4, 4, params4x4);
  ...
  for (var y = 0; y < 4; y++) {
    for (var x = 0; x < 4; x++) {
      ...
      final double pos = (_afvFreqs[y * 4 + x] - low) / (high - low);
...
final class HfGlobal {
  /// Stores the params value used while processing JPEG XL data.
  ///
  late List<DctParams> params;

  /// Processes the weights ready data used by the JPEG XL codec.
  ///
  final List<bool> _weightsReady = List<bool>.filled(17, false);

  /// Stores the weights value used while processing JPEG XL data.
  ///
  late final List<List<List<Float32List>?>> weights;

  /// Stores the weights flat value used while processing JPEG XL data.
  ///
  late final List<List<Float32List?>> weightsFlat;

  /// Stores the weights flat v value used while processing JPEG XL data.
  ///
  late final List<List<Float32x4List?>> weightsFlatV;

  /// Stores the weights flat tV value used while processing JPEG XL data.
  ///
  late final List<List<Float32x4List?>> weightsFlatTV;

  /// Stores the weights width value used while processing JPEG XL data.
  ///
  late final List<int> weightsWidth;

  /// Stores the num hf presets value used while processing JPEG XL data.
  ///
  late final int numHfPresets;

  /// Creates Hf global data for JPEG XL processing.
  ///
  HfGlobal({
    required BitReader reader,
    required Frame frame,
  }) {
    final bool quantAllDefault = reader.readBool();
    if (quantAllDefault) {
    ...
    weightsFlatV = List.generate(17, (_) => List<Float32x4List?>.filled(3, null));
    weightsFlatTV = List.generate(17, (_) => List<Float32x4List?>.filled(3, null));
  ...
  List<Float32List?> flatWeightsFor(int index) {
    if (!_weightsReady[index]) {
      ...
      for (var ch = 0; ch < 3; ch++) {
        ...
        weightsFlatV[index][ch] = Float32x4List.view(flat.buffer, 0, flat.length >> 2);
        ...
        weightsFlatTV[index][ch] = Float32x4List.view(flatT.buffer, 0, flatT.length >> 2);
      }
  ...
  /// Processes the setup dct param data used by the JPEG XL codec.
  ///
  DctParams _setupDctParam(BitReader reader, Frame frame, int index) {
    ...
    switch (encodingMode) {
      ...
      case TransformMode.hornuss:
        ...
        return DctParams(dctParam: null, param: m, mode: encodingMode);
      ...
      case TransformMode.dct4:
        ...
        return DctParams(dctParam: _readDCTParams(reader), param: m, mode: encodingMode);
      case TransformMode.dct:
        return DctParams(dctParam: _readDCTParams(reader), param: null, mode: encodingMode);
      case TransformMode.raw:
        final double den = reader.readF16();
        final TransformType tt = TransformType.byParameterIndex(index);
        final info = [
          ModularChannel(height: tt.matrixHeight, width: tt.matrixWidth, vshift: 0, hshift: 0),
          ModularChannel(height: tt.matrixHeight, width: tt.matrixWidth, vshift: 0, hshift: 0),
          ModularChannel(height: tt.matrixHeight, width: tt.matrixWidth, vshift: 0, hshift: 0),
        ];
        final stream = ModularStream.read(reader: reader, ctx: frame.modularContext, streamIndex: 1 + 3 * frame.numLfGroups + index, channelArray: info);
        ...
        return DctParams(dctParam: null, param: m, mode: encodingMode, denominator: den);
      ...
      case TransformMode.afv:
        ...
        final List<List<double>> d = _readDCTParams(reader);
        final List<List<double>> f = _readDCTParams(reader);
        return DctParams(dctParam: d, param: m, mode: encodingMode, params4x4: f);
      default:
        throw const JpegXlInvalidBitstreamException(message: 'invalid quant mode');
    }
  }

  /// Processes the get afvtransform weights data used by the JPEG XL codec.
  ///
  List<Float32List> _getAFVTransformWeights(int index, int c) {
    final DctParams p = params[index];
    return getAFVQuantWeights(p.dctParam![c], p.params4x4![c], p.param![c]);
  }

  /// Processes the generate weights data used by the JPEG XL codec.
  ///
  void _generateWeights(int index) {
    ...
    for (var c = 0; c < 3; c++) {
      switch (p.mode) {
        case TransformMode.dct:
          weights[index][c] = getDCTQuantWeights(tt.matrixHeight, tt.matrixWidth, p.dctParam![c]);
        case TransformMode.dct4:
          weights[index][c] = getDct4x4QuantWeights(p.dctParam![c], p.param![c]);
        case TransformMode.dct2:
          weights[index][c] = getDct2x2QuantWeights(p.param![c]);
        case TransformMode.hornuss:
          weights[index][c] = getHornussQuantWeights(p.param![c]);
        case TransformMode.dct4x8:
          weights[index][c] = getDct4x8QuantWeights(p.dctParam![c], p.param![c][0]);
        case TransformMode.afv:
