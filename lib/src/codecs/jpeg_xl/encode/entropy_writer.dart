...
import 'package:imcodec/src/codecs/jpeg_xl/encode/ans_writer.dart';
...
/// Processes the level0 codes data used by the JPEG XL codec.
///
const _level0Codes = <int, (int, int)>{0: (0, 2), 4: (1, 2), 3: (2, 2), 2: (3, 3), 1: (7, 4), 5: (15, 4)};
...
void _writeComplexPrefixCode(BitWriter w, List<int> lengths) {
  ...
  var totalCode = 0;
  for (var j = 0; j < 18; j++) {
    final int len = level1Lengths[_codelenMap[j]];
    final (int code, int bits) = _level0Codes[len]!;
...
(List<int>, List<int>) writePrefixCode(BitWriter w, List<int> counts, int alphabetSize) {
  ...
  final List<int> lens = huffmanLengths(counts, 15);
...
final class Lz77Ops {
  /// Stores the kinds value used while processing JPEG XL data.
  ///
  final List<int> kinds = []; // 0 = literal, 1 = match
  /// Stores the ctxs value used while processing JPEG XL data.
  ///
  final List<int> ctxs = []; // context of the pixel at this position
  /// Stores the a value used while processing JPEG XL data.
  ///
  final List<int> a = []; // literal value / match length
  /// Stores the b value used while processing JPEG XL data.
  ///
  final List<int> b = []; // unused / match distance
}

/// Stores the lz77 min symbol value used while processing JPEG XL data.
///
const lz77MinSymbol = 224;

/// Stores the lz77 min length value used while processing JPEG XL data.
///
const lz77MinLength = 3;

/// Processes lz77 length config information in a JPEG XL codestream.
///
const lz77LengthConfig = HybridIntegerConfig(splitExponent: 4, msbInToken: 0, lsbInToken: 0);
...
enum Lz77Effort {
  ...
  /// Processes shallow information in a JPEG XL codestream.
  ///
  shallow(chainDepth: 24, niceLength: 4096, lazy: false),

  /// Processes deep information in a JPEG XL codestream.
  ///
  deep(chainDepth: 256, niceLength: 512, lazy: true);

  /// Stores the chain depth value used while processing JPEG XL data.
  ///
  final int chainDepth;

  /// Stores the nice length value used while processing JPEG XL data.
  ///
  final int niceLength;

  /// Stores the lazy value used while processing JPEG XL data.
  ///
  final bool lazy;

  /// Creates an LZ77 search-effort configuration.
  ///
  const Lz77Effort({required this.chainDepth, required this.niceLength, required this.lazy});
}
...
Lz77Ops lz77Compress(List<int> contexts, List<int> values, {Lz77Effort effort = Lz77Effort.shallow}) {
  final ops = Lz77Ops();
  final int n = values.length;
  const hashBits = 15;
  final int chainDepth = effort.chainDepth;
  final int niceLength = effort.niceLength;
  final bool lazy = effort.lazy;
  ...
  while (i < n) {
    ...
    if (mLen >= lz77MinLength) {
      ...
      final int next = lazy && i + 1 < n ? findMatch(i + 1) : 0;
      if ((next >> 24) > mLen) {
        emitLiteral(i);
        i++;
        pending = next;
...
final class EntropyCodes {
  /// Stores the config value used while processing JPEG XL data.
  ///
  ...
  /// Stores the codes state used internally by the JPEG XL codec.
  ///
  final List<List<int>> _codes;

  /// Stores the lens state used internally by the JPEG XL codec.
  ///
  final List<List<int>> _lens;

  /// Stores the alphabet sizes state used internally by the JPEG XL codec.
  ///
  final List<int> _alphabetSizes;

  /// Stores the histograms state used internally by the JPEG XL codec.
  ///
  final List<List<int>> _histograms;

  /// Stores the sparse state used internally by the JPEG XL codec.
  ///
  ...
  /// Stores the extra bits state used internally by the JPEG XL codec.
  ///
  double _extraBits = 0;

  /// Stores the ans tables state used internally by the JPEG XL codec.
  ///
  List<AnsAliasTable>? _ansTables;

  /// Stores the ans log state used internally by the JPEG XL codec.
  ///
  int _ansLog = 8;

  /// Creates Entropy codes state for JPEG XL processing.
  ///
  EntropyCodes._({
  ...
  factory EntropyCodes.buildLz77({
    required int numContexts,
    required List<Lz77Ops> sections,
    required HybridIntegerConfig config,
  }) {
    final codes = EntropyCodes._(config: config, codes: [], lens: [], alphabetSizes: [], histograms: [], numContexts: numContexts)..usesLz77 = true;
    for (final ops in sections) {
      for (var i = 0; i < ops.kinds.length; i++) {
        if (ops.kinds[i] == 0) {
          final (int token, int nbits, _) = tokenizeHybrid(config, ops.a[i]);
          assert(token < lz77MinSymbol, 'literal token collides with LZ77 length symbols');
          codes._count(ops.ctxs[i], token, nbits);
        } else {
          final (int lt, int lnbits, _) = tokenizeHybrid(lz77LengthConfig, ops.a[i] - lz77MinLength);
          codes._count(ops.ctxs[i], lz77MinSymbol + lt, lnbits);
          final (int dt, int dnbits, _) = tokenizeHybrid(config, ops.b[i] + 119);
  ...
  /// Stores the num clusters state used internally by the JPEG XL codec.
  ///
  int get _numClusters => usesLz77 ? numContexts + 1 : numContexts;

  /// Processes the count data used by the JPEG XL codec.
  ///
  ...
  double estimatedBits() {
    ...
    for (final List<int> hist in _histograms) {
      final List<int>? lens = _safeLengths(hist);
      if (lens == null) {
  ...
  /// Stores the ans log alphabet size state used internally by the JPEG XL codec.
  ///
  ...
  double ansEstimatedBits() {
    ...
    for (final List<int> hist in _histograms) {
      ...
      final Int32List freqs = normalizeFrequencies(hist, hist.length);
      for (var s = 0; s < hist.length; s++) {
        if (hist[s] > 0) {
  ...
  void writeAnsHeader(BitWriter w) {
    ...
    final int clusters = _numClusters;
    ...
    if (usesLz77) {
      ...
      if (lz77LengthConfig.splitExponent != 8) {
        w.writeBits(lz77LengthConfig.msbInToken, ceilLog1p(lz77LengthConfig.splitExponent));
        w.writeBits(lz77LengthConfig.lsbInToken, ceilLog1p(lz77LengthConfig.splitExponent - lz77LengthConfig.msbInToken));
      }
    }
    ...
    for (var c = 0; c < clusters; c++) {
      ...
      final Int32List freqs = normalizeFrequencies(counts, declared);
      writeAnsDistribution(w, freqs);
      _ansTables!.add(AnsAliasTable(freqs: freqs, logAlphabetSize: _ansLog));
    }
  }
  ...
  void encodeAnsLz77Section(BitWriter w, Lz77Ops ops) {
    ...
    for (var i = 0; i < ops.kinds.length; i++) {
      if (ops.kinds[i] == 0) {
        final (int token, int nbits, int extra) = tokenizeHybrid(config, ops.a[i]);
        enc.add(ops.ctxs[i], token, extra: extra, extraBits: nbits);
      } else {
        final (int lt, int lnbits, int lextra) = tokenizeHybrid(lz77LengthConfig, ops.a[i] - lz77MinLength);
        enc.add(ops.ctxs[i], lz77MinSymbol + lt, extra: lextra, extraBits: lnbits);
        final (int dt, int dnbits, int dextra) = tokenizeHybrid(config, ops.b[i] + 119);
  ...
  /// Processes the finish histograms data used by the JPEG XL codec.
  ///
  ...
  static void _writeClusterMap(BitWriter w, int clusters) {
    ...
    final nested = EntropyWriter(numContexts: 1);
    for (var i = 0; i < clusters; i++) {
      nested.write(0, i);
    }
    nested.finalize(w);
  ...
  void writeOps(BitWriter w, Lz77Ops ops) {
    ...
    for (var i = 0; i < ops.kinds.length; i++) {
      if (ops.kinds[i] == 0) {
        ...
        final (int t, int nbits, int extra) = tokenizeHybrid(lz77LengthConfig, ops.a[i] - lz77MinLength);
...
final class EntropyWriter {
  /// Stores the num contexts value used while processing JPEG XL data.
  ///
  final int numContexts;
  ...
  /// Stores the contexts state used internally by the JPEG XL codec.
  ///
  final List<int> _contexts = [];

  /// Stores the values state used internally by the JPEG XL codec.
  ///
  final List<int> _values = [];

  /// Creates Entropy writer data for JPEG XL processing.
  ///
  EntropyWriter({
    required this.numContexts,
  ...
  /// Processes write information in a JPEG XL codestream.
  ///
  void write(int context, int value) {
    assert(context >= 0 && context < numContexts, 'The entropy context is outside the configured range.');
    assert(value >= 0, 'Entropy values cannot be negative.');
    _contexts.add(context);
    _values.add(value);
  }

  /// Processes finalize information in a JPEG XL codestream.
  ///
  void finalize(BitWriter w) {
    final codes = EntropyCodes.build(numContexts: numContexts, contexts: _contexts, values: _values, config: config);
    codes.writeHeader(w);
