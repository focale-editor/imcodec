...
/// Stores the rct wp properties value used while processing JPEG XL data.
///
...
/// Processes the log2 data used by the JPEG XL codec.
///
...
///
final class _Node {
  /// Stores the prop index state used by the JPEG XL codec.
  ///
  int propIndex = -1; // index into treeProperties; < 0 => leaf
  /// Stores the value state used by the JPEG XL codec.
  ///
  int value = 0;

  /// Stores the context state used by the JPEG XL codec.
  ///
  int context = -1;

  /// Stores the left state used by the JPEG XL codec.
  ///
  _Node? left;

  /// Stores the right state used by the JPEG XL codec.
  ///
  _Node? right;

  /// Stores the samples state used by the JPEG XL codec.
  ///
  List<int> samples; // sample indices (training)
  /// Stores the split prop state used by the JPEG XL codec.
  ///
  int splitProp = -2; // -2 = not computed, -1 = no useful split
  /// Stores the split thr state used by the JPEG XL codec.
  ///
  int splitThr = 0;

  /// Stores the split gain state used by the JPEG XL codec.
  ///
  double splitGain = 0;
...
final class ContextTree {
  /// Stores the root state used internally by the JPEG XL codec.
  ///
  final _Node _root;
  ...
  /// Stores the nodes in order state used internally by the JPEG XL codec.
  ///
  ...
  /// Creates Context tree state for JPEG XL processing.
  ///
  ContextTree._({
    required this._root,
    required this.numContexts,
    required this._nodesInOrder,
...
/// Processes the counts entropy data used by the JPEG XL codec.
///
...
ContextTree learnContextTree(Int32List props, Int32List tokens, List<int> properties, {int maxContexts = 64, double minGainBits = 96}) {
  ...
  void computeSplit(_Node node) {
    ...
    if (node.splitGain < minGainBits) {
      node.splitProp = -1;
    }
  }

  final root = _Node(samples: [for (var i = 0; i < n; i++) i]);
  computeSplit(root);
  var leaves = 1;
  ...
  while (leaves < maxContexts) {
    ...
    final int pi = pick.splitProp;
    final int thr = pick.splitThr;
    final leftS = <int>[];
    final rightS = <int>[];
    ...
    pick
      ..propIndex = pi
      ..value = thr
      ..left = _Node(samples: leftS)
      ..right = _Node(samples: rightS)
      ..samples = const []
      ..splitProp = -1;
  ...
  while (queue.isNotEmpty) {
    final _Node node = queue.removeAt(0);
    ordered.add(node);
    if (node.propIndex < 0) {
  ...
  return ContextTree._(root: root, numContexts: nextContext, nodesInOrder: ordered, properties: properties, trainingBits: trainingBits);
}
...
void serializeContextTree(BitWriter w, ContextTree tree, int predictor, [List<int>? leafPredictors]) {
  final tokens = EntropyWriter(numContexts: 6);
  for (final _Node node in tree._nodesInOrder) {
    if (node.propIndex < 0) {
