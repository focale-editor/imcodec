...
import 'package:imcodec/src/codecs/jpeg_xl/encode/context_tree.dart';
import 'package:imcodec/src/codecs/jpeg_xl/encode/entropy_writer.dart';
import 'package:imcodec/src/codecs/jpeg_xl/encode/headers.dart';
import 'package:imcodec/src/codecs/jpeg_xl/encode/var_dct/xyb_forward.dart';
import 'package:imcodec/src/codecs/jpeg_xl/encode/wp_predictor.dart';
...
import 'package:imcodec/src/codecs/jpeg_xl/var_dct/hf_coefficients.dart' show HfCoefficients;
import 'package:imcodec/src/codecs/jpeg_xl/var_dct/hf_global.dart'
    show DctParams, defaultDctParams, getAFVQuantWeights, getDCTQuantWeights, getDct2x2QuantWeights, getDct4x4QuantWeights, getDct4x8QuantWeights, getHornussQuantWeights;
import 'package:imcodec/src/codecs/jpeg_xl/var_dct/hf_pass.dart' show getNaturalOrder;
...
/// `var_dct/hf_coefficients.dart` and `var_dct/lf_coefficients.dart`).
...
class VarDctConfiguration {
  ...
  /// Creates Var dct configuration data for JPEG XL processing.
  ///
  ...
  factory VarDctConfiguration.fromDistance({
    ...
    final int quantLF = (16 / distance).round().clamp(1, 65536);
    return VarDctConfiguration(quantLF: quantLF, acScale: 1.0 / distance);
  }
}

/// Processes the pack signed data used by the JPEG XL codec.
///
int _packSigned(int v) => v >= 0 ? v << 1 : (-v << 1) - 1;

/// Processes the hf config data used by the JPEG XL codec.
///
const _hfConfig = HybridIntegerConfig(splitExponent: 4, msbInToken: 1, lsbInToken: 0);
...
/// Processes the tt8 data used by the JPEG XL codec.
///
final TransformType _tt8 = TransformType.byType(0); // DCT 8x8: orderID 0, parameterIndex 0
/// Processes the tt16 data used by the JPEG XL codec.
///
final TransformType _tt16 = TransformType.byType(4); // DCT 16x16: orderID 2, parameterIndex 4
/// Processes the tt32 data used by the JPEG XL codec.
///
final TransformType _tt32 = TransformType.byType(5); // DCT 32x32: orderID 3, parameterIndex 5
/// Processes the tt64 data used by the JPEG XL codec.
///
final TransformType _tt64 = TransformType.byType(18); // DCT 64x64: orderID 7, parameterIndex 11
/// Processes the tt128 data used by the JPEG XL codec.
///
final TransformType _tt128 = TransformType.byType(21); // DCT 128x128: orderID 9, parameterIndex 13
/// Processes the tt256 data used by the JPEG XL codec.
///
final TransformType _tt256 = TransformType.byType(24); // DCT 256x256: orderID 11, parameterIndex 15
/// Processes the tt16x8 data used by the JPEG XL codec.
///
final TransformType _tt16x8 = TransformType.byType(6); // DCT 16x8: orderID 4, parameterIndex 6, flip
/// Processes the tt8x16 data used by the JPEG XL codec.
///
final TransformType _tt8x16 = TransformType.byType(7); // DCT 8x16: orderID 4, parameterIndex 6, no flip
/// Processes the tt32x8 data used by the JPEG XL codec.
///
final TransformType _tt32x8 = TransformType.byType(8); // DCT 32x8: orderID 5, parameterIndex 7, flip
/// Processes the tt8x32 data used by the JPEG XL codec.
///
final TransformType _tt8x32 = TransformType.byType(9); // DCT 8x32: orderID 5, parameterIndex 7, no flip
/// Processes the tt32x16 data used by the JPEG XL codec.
///
final TransformType _tt32x16 = TransformType.byType(10); // DCT 32x16: orderID 6, parameterIndex 8, flip
/// Processes the tt16x32 data used by the JPEG XL codec.
///
final TransformType _tt16x32 = TransformType.byType(11); // DCT 16x32: orderID 6, parameterIndex 8, no flip
/// Processes the tt64x32 data used by the JPEG XL codec.
///
final TransformType _tt64x32 = TransformType.byType(19); // DCT 64x32: orderID 8, parameterIndex 12, flip
/// Processes the tt32x64 data used by the JPEG XL codec.
///
final TransformType _tt32x64 = TransformType.byType(20); // DCT 32x64: orderID 8, parameterIndex 12, no flip
/// Processes the tt128x64 data used by the JPEG XL codec.
///
final TransformType _tt128x64 = TransformType.byType(22); // DCT 128x64: orderID 10, parameterIndex 14, flip
/// Processes the tt64x128 data used by the JPEG XL codec.
///
final TransformType _tt64x128 = TransformType.byType(23); // DCT 64x128: orderID 10, parameterIndex 14, no flip
/// Processes the tt256x128 data used by the JPEG XL codec.
///
final TransformType _tt256x128 = TransformType.byType(25); // DCT 256x128: orderID 12, parameterIndex 16, flip
/// Processes the tt128x256 data used by the JPEG XL codec.
///
final TransformType _tt128x256 = TransformType.byType(26); // DCT 128x256: orderID 12, parameterIndex 16, no flip
/// Processes the tt4x4 data used by the JPEG XL codec.
///
final TransformType _tt4x4 = TransformType.byType(3); // DCT4x4 (Tranche C, bespoke): orderID 1, parameterIndex 3, TransformMethod.dct4
/// Processes the tt hornuss data used by the JPEG XL codec.
///
final TransformType _ttHornuss = TransformType.byType(1); // Hornuss (Tranche C, bespoke): orderID 1, parameterIndex 1, TransformMethod.hornuss
/// Processes the tt dct2x2 data used by the JPEG XL codec.
///
final TransformType _ttDct2x2 = TransformType.byType(2); // DCT2x2 (Tranche C, bespoke): orderID 1, parameterIndex 2, TransformMethod.dct2
/// Processes the tt dct4x8 data used by the JPEG XL codec.
///
final TransformType _ttDct4x8 = TransformType.byType(12); // DCT4x8 (Tranche C, bespoke): orderID 1, parameterIndex 9, TransformMethod.dct4x8
/// Processes the tt dct8x4 data used by the JPEG XL codec.
///
final TransformType _ttDct8x4 = TransformType.byType(13); // DCT8x4 (Tranche C, bespoke): orderID 1, parameterIndex 9, TransformMethod.dct8x4
/// Processes the tt afv0 data used by the JPEG XL codec.
///
final TransformType _ttAfv0 = TransformType.byType(14); // AFV0 (Tranche C, bespoke): orderID 1, parameterIndex 10, TransformMethod.afv
/// Processes the tt afv1 data used by the JPEG XL codec.
///
final TransformType _ttAfv1 = TransformType.byType(15); // AFV1 (Tranche C, bespoke): orderID 1, parameterIndex 10, TransformMethod.afv
/// Processes the tt afv2 data used by the JPEG XL codec.
///
final TransformType _ttAfv2 = TransformType.byType(16); // AFV2 (Tranche C, bespoke): orderID 1, parameterIndex 10, TransformMethod.afv
/// Processes the tt afv3 data used by the JPEG XL codec.
///
final TransformType _ttAfv3 = TransformType.byType(17); // AFV3 (Tranche C, bespoke): orderID 1, parameterIndex 10, TransformMethod.afv
...
final List<TransformType> _cascadeSizes = [_tt32, _tt64, _tt128, _tt256];
...
final List<(TransformType, TransformType)> _cascadeRectPairs = [(_tt16x32, _tt32x16), (_tt32x64, _tt64x32), (_tt64x128, _tt128x64), (_tt128x256, _tt256x128)];
...
Uint8List encodeLossyVarDct(Uint8List rgbPixels, {required int width, required int height, VarDctConfiguration config = const VarDctConfiguration()}) {
  ...
  planes.forEach(XybForward.srgbToLinear);
  ...
  DctParams customParams(TransformType tt) {
    final DctParams base = defaultDctParams[tt.parameterIndex];
    switch (base.mode) {
      case TransformMode.dct:
        final List<List<double>> scaledDctParam = [
          for (var c = 0; c < 3; c++) [base.dctParam![c][0] * config.acScale, ...base.dctParam![c].skip(1)],
        ];
        return DctParams(dctParam: scaledDctParam, param: null, mode: TransformMode.dct);
      case TransformMode.dct4:
        final List<List<double>> scaledDctParam = [
          for (var c = 0; c < 3; c++) [base.dctParam![c][0] * config.acScale, ...base.dctParam![c].skip(1)],
        ];
        return DctParams(dctParam: scaledDctParam, param: base.param, mode: TransformMode.dct4);
      case TransformMode.hornuss:
        ...
        return DctParams(
          dctParam: null,
          param: [
            for (var c = 0; c < 3; c++) [for (final v in base.param![c]) v * config.acScale],
          ],
      ...
      case TransformMode.dct4x8:
        ...
        return DctParams(dctParam: scaledDctParam, param: base.param, mode: TransformMode.dct4x8);
      case TransformMode.afv:
        ...
        final List<List<double>> scaledParams4x4 = [
          for (var c = 0; c < 3; c++) [base.params4x4![c][0] * config.acScale, ...base.params4x4![c].skip(1)],
        ];
        final List<List<double>> scaledParam = [
          for (var c = 0; c < 3; c++)
            [
              for (var i = 0; i < base.param![c].length; i++)
                if (i < 6) base.param![c][i] * config.acScale else base.param![c][i],
            ],
        ];
        return DctParams(dctParam: scaledDctParam, param: scaledParam, mode: TransformMode.afv, params4x4: scaledParams4x4);
      default:
        throw UnsupportedError('transform mode ${base.mode} not yet supported by the lossy encoder');
    }
  }
  ...
  List<List<Float32List>> rawWeightsFor(TransformType tt, DctParams p) {
    switch (p.mode) {
      case TransformMode.dct:
        return [for (var c = 0; c < 3; c++) getDCTQuantWeights(tt.pixelHeight, tt.pixelWidth, p.dctParam![c])];
      case TransformMode.dct4:
        return [for (var c = 0; c < 3; c++) getDct4x4QuantWeights(p.dctParam![c], p.param![c])];
      case TransformMode.dct4x8:
        return [for (var c = 0; c < 3; c++) getDct4x8QuantWeights(p.dctParam![c], p.param![c][0])];
      case TransformMode.afv:
        return [for (var c = 0; c < 3; c++) getAFVQuantWeights(p.dctParam![c], p.params4x4![c], p.param![c])];
      case TransformMode.hornuss:
        return [for (var c = 0; c < 3; c++) getHornussQuantWeights(p.param![c])];
      case TransformMode.dct2:
        return [for (var c = 0; c < 3; c++) getDct2x2QuantWeights(p.param![c])];
      default:
        throw UnsupportedError('transform mode ${p.mode} not yet supported by the lossy encoder');
    }
  }
  ...
  final Map<int, DctParams> customParamsByType = {for (final tt in _activeTransformTypes) tt.type: customParams(tt)};
  final Map<int, List<List<Float32List>>> rawWeightByType = {for (final tt in _activeTransformTypes) tt.type: rawWeightsFor(tt, customParamsByType[tt.type]!)};
  final double globalScaleF = 65536.0 / config.globalScale;
  final List<double> scaleFactor = [globalScaleF * math.pow(0.8, config.xqmScale - 2.0), globalScaleF, globalScaleF * math.pow(0.8, config.bqmScale - 2.0)];
  const List<double> lfDequantDefault = [1 / 4096.0, 1 / 512.0, 1 / 256.0];
  final List<double> sd = [for (var c = 0; c < 3; c++) (1 << 16) * lfDequantDefault[c] / (config.globalScale * config.quantLF)];
  ...
  final _ChromaFromLumaFit cfl = _chromaFromLumaFit(planes, bh, bw, cflScratchA, cflScratchB);
  ...
  final double refStep = scaleFactor[1] / rawWeightByType[_tt8.type]![1][0][1];
  ...
  final Map<int, _TransformCtx> ctxByType = {for (final tt in _activeTransformTypes) tt.type: _TransformCtx(tt: tt, hfctx: hfctx, rawWeight: rawWeightByType[tt.type]!)};
  ...
  final int numGroups = groupsX * groupsY;
  ...
  if (config.enableVariableTransforms) {
    ...
    final List<(List<_PlacedBlock>, List<Int32List>)> candidates = _decideTransformLayout(
      planes,
      cfl,
      refStep,
      sd,
    ...
    Uint8List assemble(List<_PlacedBlock> blocks, List<Int32List> dcInt) => _finishEncode(
      blocks,
      dcInt,
      bh,
      bw,
    ...
    if (const bool.fromEnvironment('jxl.encdebug')) {
      ...
      for (final _PlacedBlock block in candidates[chosenLevel].$1) {
  ...
  final List<_PlacedBlock> placedBlocks = [
    for (var by = 0; by < bh; by++)
      for (var bx = 0; bx < bw; bx++) _PlacedBlock(by: by, bx: bx, tt: _tt8),
  ];
  for (final block in placedBlocks) {
    block.computeAndQuantize(planes, cfl, refStep, sd, ctxByType[_tt8.type]!.rawWeight, scaleFactor, dcInt, bw, scratchA, scratchB);
  }
...
Uint8List _finishEncode(
  ...
  final Set<int> usedTypeIndices = {for (final block in placedBlocks) block.tt.type};
  final Map<int, DctParams>? customParamsByIndex = customParamsByType == null
      ? null
      : {
          for (final tt in _activeTransformTypes)
            if (usedTypeIndices.contains(tt.type)) tt.parameterIndex: customParamsByType[tt.type]!,
        };

  final blocksByGroup = List<List<_PlacedBlock>>.generate(numGroups, (_) => []);
  for (final block in placedBlocks) {
    final int g = (block.by ~/ 32) * groupsX + (block.bx ~/ 32);
  ...
  if (config.enableRdoq) {
    _chooseAcRdoq(placedBlocks, planes, cfl, config.acScale, scaleFactor, scratchA, scratchB, hfctx, ctxByType, groupsX, groupsY, blocksByGroup, config.rdoqLambdaOverride);
  }
  ...
  final groupTokens = <_GroupTokens>[
    for (var gy = 0; gy < groupsY; gy++)
      for (var gx = 0; gx < groupsX; gx++) _computeGroupTokens(gy * 32, gx * 32, blocksByGroup[gy * groupsX + gx], hfctx, ctxByType),
  ];
  final _AcClustering clustering = _chooseAcClustering(groupTokens);
  ...
  final int numLfGroups = lfGroupsX * lfGroupsY;
  ...
  final blocksByLfGroup = List<List<_PlacedBlock>>.generate(numLfGroups, (_) => []);
  for (final block in placedBlocks) {
    final int id = (block.by ~/ _lfGroupBlockDim) * lfGroupsX + (block.bx ~/ _lfGroupBlockDim);
  ...
  if (numGroups == 1 && numLfGroups == 1) {
    ...
    final lfGroupSections = <Uint8List>[
      for (var id = 0; id < numLfGroups; id++) _assembleLfGroupSection(dcInt, bw, lfGroupOriginBy[id], lfGroupOriginBx[id], lfGroupBh[id], lfGroupBw[id], blocksByLfGroup[id], cfl),
    ];
    ...
    final List<Uint8List> passGroupSections = [for (var g = 0; g < numGroups; g++) _assembleGroupSection(clustering.codes, clustering.mappedClustersPerGroup[g], groupTokens[g].values)];
    ...
    if (const bool.fromEnvironment('jxl.encdebug')) {
      ...
      // ignore: avoid_print
      print(
        'vardct sections: lfGlobal=${lfGlobalW.toBytes().length} '
...
Uint8List _assembleLfGroupSection(List<Int32List> dcInt, int bwFull, int originBy, int originBx, int lfGroupBh, int lfGroupBw, List<_PlacedBlock> blocksInLfGroup, _ChromaFromLumaFit cfl) {
  ...
  _writeHfMetadata(w, lfGroupBh, lfGroupBw, blocksInLfGroup, originBy, originBx, cfl);
  if (const bool.fromEnvironment('jxl.encdebug')) {
    // ignore: avoid_print
...
typedef _Scored = ({_PlacedBlock block, ({List<double> dc, List<Int32List> ac, double distortion}) quant, int mult, double rate, double cost});

/// Processes the decide transform layout data used by the JPEG XL codec.
///
List<(List<_PlacedBlock>, List<Int32List>)> _decideTransformLayout(
  ...
  final _TransformCtx ctx8 = ctxByType[_tt8.type]!;
  ...
  for (final block in bootstrapBlocks) {
    block.computeAndQuantize(planes, cfl, refStep, sd, ctx8.rawWeight, scaleFactor, dcIntBootstrap, bw, scratchA, scratchB);
  }
  final bootstrapByGroup = List<List<_PlacedBlock>>.generate(groupsX * groupsY, (_) => []);
  for (final block in bootstrapBlocks) {
    final int g = (block.by ~/ 32) * groupsX + (block.bx ~/ 32);
  ...
  final bootstrapTokens = <_GroupTokens>[
    for (var gy = 0; gy < groupsY; gy++)
      for (var gx = 0; gx < groupsX; gx++) _computeGroupTokens(gy * 32, gx * 32, bootstrapByGroup[gy * groupsX + gx], hfctx, ctxByType, grid: liveGrid[gy * groupsX + gx]),
  ];
  final _AcClustering bootstrap = _chooseAcClustering(bootstrapTokens);
  ...
  Int32List predictedForPosition(int by, int bx) {
    final int groupX = bx ~/ 32;
    final int groupY = by ~/ 32;
    final Int32List grid = liveGrid[groupY * groupsX + groupX];
    final int localY = by - groupY * 32;
    final int localX = bx - groupX * 32;
    return Int32List.fromList([for (var c = 0; c < 3; c++) HfCoefficients.getPredictedNonZeroes(grid, c, localY, localX)]);
  }
  ...
  void writeLiveGridCells(TransformType tt, int by, int bx, List<Int32List> ac) {
    final int groupX = bx ~/ 32;
    final int groupY = by ~/ 32;
    final Int32List grid = liveGrid[groupY * groupsX + groupX];
    final int localY = by - groupY * 32;
    final int localX = bx - groupX * 32;
    final _TransformCtx ctx = ctxByType[tt.type]!;
    final int numBlocks = ctx.numBlocks;
    for (var c = 0; c < 3; c++) {
      final (_, int countNonZero, _) = _scanChannelValues(ctx, ac[c]);
      final int fill = (countNonZero + numBlocks - 1) ~/ numBlocks;
  ...
  List<int> snapshotGridCells(TransformType tt, int by, int bx) {
    ...
    return [
      for (var c = 0; c < 3; c++)
        for (var iy = 0; iy < tt.dctSelectHeight; iy++)
          for (var ix = 0; ix < tt.dctSelectWidth; ix++) grid[c * 1024 + (localY + iy) * 32 + (localX + ix)],
    ];
  }
  ...
  _Scored scoreFreshCandidate(int by, int bx, TransformType tt, double lambdaForLevel) {
    final _TransformCtx ctx = ctxByType[tt.type]!;
    final candidate = _PlacedBlock(by: by, bx: bx, tt: tt);
    final List<List<Float32List>> coeffBuf = candidate.computeCoeffBuf(planes, cfl, scratchA, scratchB);
    ...
    final double rate = _blockRate(ctx, hfctx, quant.ac, predictedForPosition(by, bx), clusterMap, lengths);
    final double cost = quant.distortion + lambdaForLevel * rate;
    return (block: candidate, quant: quant, mult: mult, rate: rate, cost: cost);
  }

  (List<_PlacedBlock>, List<Int32List>, bool) tryMergeLevel(List<_PlacedBlock> layoutIn, List<Int32List> dcIntIn, TransformType targetType, double lambdaForLevel) {
    ...
    for (final block in layoutIn) {
      final int blockH = block.tt.dctSelectHeight;
      final int blockW = block.tt.dctSelectWidth;
    ...
    double blockRateIn(_PlacedBlock block) => _blockRate(ctxByType[block.tt.type]!, hfctx, block.acInt, predictedForPosition(block.by, block.bx), clusterMap, lengths);
    ...
    for (var by = 0; by < bh; by++) {
      for (var bx = 0; bx < bw; bx++) {
        ...
        final bool canPair = by % strideY == 0 && bx % strideX == 0 && by + strideY - 1 < bh && bx + strideX - 1 < bw;
        _PlacedBlock? merged;
        if (canPair) {
          ...
          for (var dy = 0; dy < strideY; dy++) {
            for (var dx = 0; dx < strideX; dx++) {
              final _PlacedBlock block = layoutBlockAt[(by + dy) * bw + (bx + dx)]!;
              if (block.by < by || block.bx < bx || block.by + block.tt.dctSelectHeight > by + strideY || block.bx + block.tt.dctSelectWidth > bx + strideX) {
                mergeable = false;
                break outer;
              }
              if (seen.add(block)) {
          ...
          if (mergeable) {
            final _Scored scored = scoreFreshCandidate(by, bx, targetType, lambdaForLevel);
            if (scored.cost < costIn) {
              assert(
                scored.block.by % targetType.dctSelectHeight == 0 && scored.block.bx % targetType.dctSelectWidth == 0,
        ...
        if (merged != null) {
          ...
          final _PlacedBlock block = layoutBlockAt[by * bw + bx]!;
  ...
  (List<_PlacedBlock>, List<Int32List>) decideLevel0(bool enableBespokeTransforms) {
    if (!enableBespokeTransforms) {
      return (bootstrapBlocks, dcIntBootstrap);
    }
    final List<TransformType> types = [_tt8, _ttHornuss, _ttDct2x2, _tt4x4, _ttDct4x8, _ttDct8x4, _ttAfv0, _ttAfv1, _ttAfv2, _ttAfv3];
    final List<Int32List> dcIntNext = [for (final ch in dcIntBootstrap) Int32List.fromList(ch)];
    final result = <_PlacedBlock>[];
    for (var by = 0; by < bh; by++) {
      for (var bx = 0; bx < bw; bx++) {
        _Scored best = scoreFreshCandidate(by, bx, types[0], lambda);
        for (var i = 1; i < types.length; i++) {
          final _Scored scored = scoreFreshCandidate(by, bx, types[i], lambda);
  ...
  (List<_PlacedBlock>, List<Int32List>, bool) decideLevel1(List<_PlacedBlock> layout0, List<Int32List> dcInt0, bool enableRectangularTransforms) {
    final List<Int32List> dcIntNext = [for (final ch in dcInt0) Int32List.fromList(ch)];
    final result = <_PlacedBlock>[];
    var anyMerged = false;

    double splitBlockRate(_PlacedBlock b) => _blockRate(ctxByType[b.tt.type]!, hfctx, b.acInt, predictedForPosition(b.by, b.bx), clusterMap, lengths);
    ...
    for (var by = 0; by < bh; by += 2) {
      ...
      while (bx < bw) {
        ...
        final _PlacedBlock c00 = layout0[by * bw + bx];
        final _PlacedBlock c01 = layout0[by * bw + bx + 1];
        final _PlacedBlock c10 = layout0[(by + 1) * bw + bx];
        final _PlacedBlock c11 = layout0[(by + 1) * bw + bx + 1];
        ...
        final _Scored whole = scoreFreshCandidate(by, bx, _tt16, lambda);
        ...
        if (enableRectangularTransforms) {
          ...
          final _Scored left = scoreFreshCandidate(by, bx, _tt16x8, lambda);
          final List<int> leftSnapshot = snapshotGridCells(_tt16x8, by, bx);
          writeLiveGridCells(_tt16x8, by, bx, left.quant.ac);
          final _Scored right = scoreFreshCandidate(by, bx + 1, _tt16x8, lambda);
          ...
          final _Scored top = scoreFreshCandidate(by, bx, _tt8x16, lambda);
          final List<int> topSnapshot = snapshotGridCells(_tt8x16, by, bx);
          writeLiveGridCells(_tt8x16, by, bx, top.quant.ac);
          final _Scored bottom = scoreFreshCandidate(by + 1, bx, _tt8x16, lambda);
  ...
  final candidates = <(List<_PlacedBlock>, List<Int32List>)>[(bootstrapBlocks, dcIntBootstrap)];
  ...
  void runCascadeFrom(List<_PlacedBlock> startLayout, List<Int32List> startDcInt) {
    final (List<_PlacedBlock> layout1, List<Int32List> dcInt1, bool level1Merged) = decideLevel1(startLayout, startDcInt, enableRectangularTransforms);
    var layout = layout1;
    var dcInt = dcInt1;
    if (level1Merged) {
    ...
    if (enableRectangularTransforms) {
      for (final TransformType tt in [_tt8x32, _tt32x8]) {
        final (List<_PlacedBlock> next, List<Int32List> dcNext, bool changed) = tryMergeLevel(layout, dcInt, tt, lambda);
        if (changed) {
          layout = next;
          dcInt = dcNext;
    ...
    for (var idx = 0; idx < _cascadeSizes.length; idx++) {
      final TransformType tt = _cascadeSizes[idx];
      if (tt.pixelWidth > maxTransformSize) {
        break;
      }
      if (enableRectangularTransforms) {
        final (TransformType wide, TransformType tall) = _cascadeRectPairs[idx];
        for (final rtt in [wide, tall]) {
          final (List<_PlacedBlock> next, List<Int32List> dcNext, bool changed) = tryMergeLevel(layout, dcInt, rtt, lambdaBeyond16);
          if (changed) {
            layout = next;
            dcInt = dcNext;
            candidates.add((layout, dcInt));
      ...
      final (List<_PlacedBlock> next, List<Int32List> dcNext, bool changed) = tryMergeLevel(layout, dcInt, tt, lambdaBeyond16);
      if (!changed) {
  ...
  if (enableBespokeTransforms) {
    resetLiveGrid();
    final (List<_PlacedBlock> layout0, List<Int32List> dcInt0) = decideLevel0(true);
...
/// The decoder's `_auxDCT2` (`var_dct_inverter.dart`) single-stage (`s=2`,
...
/// The transpose of the decoder's `_auxDCT2` (var_dct_inverter.dart) at scale
...
void _auxDCT2Transposed(List<Float32List> coeffs, List<Float32List> result, int s) {
  for (var y = 0; y < 8; y++) {
  ...
  for (var iy = 0; iy < num; iy++) {
    for (var ix = 0; ix < num; ix++) {
      final double dA = coeffs[iy * 2][ix * 2];
      final double dB = coeffs[iy * 2][ix * 2 + 1];
      final double dC = coeffs[iy * 2 + 1][ix * 2];
      final double dD = coeffs[iy * 2 + 1][ix * 2 + 1];
...
/// Creates an internal placed block.
///
class _PlacedBlock {
  /// Stores the by state used by the JPEG XL codec.
  ///
  final int by;

  /// Stores the bx state used by the JPEG XL codec.
  ///
  final int bx;

  /// Stores the tt state used by the JPEG XL codec.
  ///
  final TransformType tt;

  /// Stores the hf mult state used by the JPEG XL codec.
  ///
  int hfMult = 1;
  ...
  List<List<Float32List>> computeCoeffBuf(List<List<Float32List>> planes, _ChromaFromLumaFit cfl, List<Float32List> scratchA, List<Float32List> scratchB) {
    final int ph = tt.pixelHeight;
    final int pw = tt.pixelWidth;
    ...
    if (tt.transformMethod == TransformMethod.dct4) {
        ...
        for (var qy = 0; qy < 2; qy++) {
          for (var qx = 0; qx < 2; qx++) {
            for (var iy = 0; iy < 4; iy++) {
              final Float32List srcRow = planes[c][by * 8 + qy * 4 + iy];
              final int srcBase = bx * 8 + qx * 4;
      ...
      for (var c = 0; c < 3; c++) {
        final List<Float32List> blockLF = List.generate(2, (_) => Float32List(2));
        for (var qy = 0; qy < 2; qy++) {
          final int baseY = by * 8 + qy * 4;
          for (var qx = 0; qx < 2; qx++) {
            final int baseX = bx * 8 + qx * 4;
            var sum = 0.0;
            for (var iy = 0; iy < 4; iy++) {
        ...
        final (double e00, double e01, double e10, double e11) = _dct4QuadrantButterfly(blockLF[0][0], blockLF[0][1], blockLF[1][0], blockLF[1][1]);
        coeffBuf[c][0][0] = e00 / 4;
        coeffBuf[c][0][1] = e01 / 4;
        coeffBuf[c][1][0] = e10 / 4;
        coeffBuf[c][1][1] = e11 / 4;
      ...
      // Verified as the exact algebraic inverse of the decoder's
      // TransformMethod.dct2 case (var_dct_inverter.dart, 3 chained _auxDCT2
      ...
      for (var c = 0; c < 3; c++) {
        for (var y = 0; y < 8; y++) {
          final Float32List row = planes[c][by * 8 + y];
          final int base = bx * 8;
      ...
      for (var c = 0; c < 3; c++) {
        ...
        for (var y = 0; y < 2; y++) {
          for (var iy = 0; iy < 4; iy++) {
            final Float32List srcRow = planes[c][by * 8 + y * 4 + iy];
            final int srcBase = bx * 8;
      ...
      for (var c = 0; c < 3; c++) {
        ...
        for (var iy = 0; iy < 4; iy++) {
          ...
          final Float32List row = planes[c][by * 8 + flipY * 4 + srcY];
          final int base = bx * 8 + flipX * 4;
          for (var ix = 0; ix < 4; ix++) {
        ...
        for (var iy = 0; iy < 4; iy++) {
          final Float32List row = planes[c][by * 8 + flipY * 4 + iy];
          final int base = bx * 8 + colOff;
          for (var ix = 0; ix < 4; ix++) {
        ...
        for (var iy = 0; iy < 4; iy++) {
          final Float32List row = planes[c][by * 8 + rowOff + iy];
          final int base = bx * 8;
          for (var ix = 0; ix < 8; ix++) {
      ...
      for (var c = 0; c < 3; c++) {
        forwardDCT2D(planes[c], coeffBuf[c], by * 8, bx * 8, 0, 0, ph, pw, scratchA, scratchB);
      }
    }

    final int llfH = tt.dctSelectHeight;
    final int llfW = tt.dctSelectWidth;
    ...
    final int regionIdx = cfl.regionIndexOf(by, bx);
    final double kXAc = cfl.kXRegion[regionIdx];
    final double kBAc = cfl.kBRegion[regionIdx];
    for (var y = 0; y < ph; y++) {
      final bool isLlfRow = y < llfH;
      for (var x = 0; x < pw; x++) {
        final double yv = coeffBuf[1][y][x];
        final bool isLlf = isLlfRow && x < llfW;
        final double kX = isLlf ? cfl.kXGlobal : kXAc;
        final double kB = isLlf ? cfl.kBGlobal : kBAc;
  ...
  void commit(({List<double> dc, List<Int32List> ac, double distortion}) candidate, int mult, List<Int32List> dcInt, int bw) {
    ...
    if (numBlocks == 1) {
      ...
      for (var c = 0; c < 3; c++) {
        final int base = c * numBlocks;
        for (var y = 0; y < llfH; y++) {
          final int row = (by + y) * bw + bx;
  ...
  _PlacedBlock copy() {
    final b = _PlacedBlock(by: by, bx: bx, tt: tt)
      ..hfMult = hfMult
  ...
  void computeAndQuantize(
    ...
    final List<List<Float32List>> coeffBuf = computeCoeffBuf(planes, cfl, scratchA, scratchB);
...
class _TransformCtx {
  ...
  /// Stores the block ctx state used by the JPEG XL codec.
  ///
  final List<int> blockCtx;

  /// Stores the hist ctx state used by the JPEG XL codec.
  ///
  late final List<int> histCtx;

  /// Stores the order state used by the JPEG XL codec.
  ///
  ...
  /// Creates an internal transform ctx.
  ///
  _TransformCtx({
    required this.tt,
    required HfBlockContext hfctx,
    required this.rawWeight,
  }) : blockCtx = [for (var c = 0; c < 3; c++) HfCoefficients.getBlockContext(hfctx, c, tt.orderID, 1, 0)],
       order = getNaturalOrder(tt.orderID) {
    histCtx = [for (final b in blockCtx) 458 * b + 37 * hfctx.numClusters];
  }

  /// Processes num blocks data for the JPEG XL codec.
  ///
  int get numBlocks => tt.dctSelectHeight * tt.dctSelectWidth;

  /// Processes order size data for the JPEG XL codec.
  ///
  int get orderSize => tt.pixelHeight * tt.pixelWidth;
}

/// Processes the assemble group section data used by the JPEG XL codec.
///
...
void _writeLfGlobal(BitWriter w, VarDctConfiguration config, double kX, double kB) {
...
class _ChromaFromLumaFit {
  /// Stores the k xGlobal state used by the JPEG XL codec.
  ///
  final double kXGlobal;

  /// Stores the k bGlobal state used by the JPEG XL codec.
  ///
  final double kBGlobal;

  /// Stores the k xRegion state used by the JPEG XL codec.
  ///
  final Float64List kXRegion;

  /// Stores the k bRegion state used by the JPEG XL codec.
  ///
  final Float64List kBRegion;

  /// Stores the corr w state used by the JPEG XL codec.
  ///
  final int corrW;

  /// Creates an internal chroma from luma fit.
  ///
  _ChromaFromLumaFit({
    required this.kXGlobal,
    required this.kBGlobal,
    required this.kXRegion,
    required this.kBRegion,
    required this.corrW,
  });

  /// Processes region index of data for the JPEG XL codec.
  ///
...
_ChromaFromLumaFit _chromaFromLumaFit(List<List<Float32List>> planes, int bh, int bw, List<Float32List> scratch0, List<Float32List> scratch1) {
  final int corrH = (bh + 7) ~/ 8;
  final int corrW = (bw + 7) ~/ 8;
  var sumYXGlobal = 0.0;
  var sumYBGlobal = 0.0;
  var sumYYGlobal = 0.0;
  ...
  for (var by = 0; by < bh; by++) {
    final int regionRow = by >> 3;
    for (var bx = 0; bx < bw; bx++) {
      final int regionIdx = regionRow * corrW + (bx >> 3);
      for (var c = 0; c < 3; c++) {
        forwardDCT2D(planes[c], coeff[c], by * 8, bx * 8, 0, 0, 8, 8, scratch0, scratch1);
      }
      for (var y = 0; y < 8; y++) {
  ...
  final (double kXGlobal, double kBGlobal) = sumYYGlobal < 1e-12 ? (0.0, 1.0) : (sumYXGlobal / sumYYGlobal, sumYBGlobal / sumYYGlobal);
  ...
  return _ChromaFromLumaFit(kXGlobal: kXGlobal, kBGlobal: kBGlobal, kXRegion: kXRegion, kBRegion: kBRegion, corrW: corrW);
}
...
void _writeModularStream(BitWriter w, List<List<int>> channelsInOrder, {int predictor = 0, ContextTree? tree, List<List<int>>? contexts}) {
  ...
  if (tree != null) {
    ...
    final treeTokens = EntropyWriter(numContexts: 6);
  ...
  final residuals = EntropyWriter(numContexts: tree?.contexts ?? 1);
...
/// the decoder's Y, X, B channel order (`var_dct/lf_coefficients.dart`'s
...
void _writeHfMetadata(BitWriter w, int bh, int bw, List<_PlacedBlock> placedBlocks, int originBy, int originBx, _ChromaFromLumaFit cfl) {
  final int nbBlocks = placedBlocks.length;
  ...
  final List<int> xFromY = [
    for (var ry = 0; ry < corrH; ry++)
      for (var rx = 0; rx < corrW; rx++) ((cfl.kXRegion[(regionRowOffset + ry) * cfl.corrW + (regionColOffset + rx)] - cfl.kXGlobal) * _colorFactor).round(),
  ];
  final List<int> bFromY = [
    for (var ry = 0; ry < corrH; ry++)
      for (var rx = 0; rx < corrW; rx++) ((cfl.kBRegion[(regionRowOffset + ry) * cfl.corrW + (regionColOffset + rx)] - cfl.kBGlobal) * _colorFactor).round(),
  ];
...
void _writeHfGlobalAndPass(BitWriter w, int numGroups, Map<int, DctParams>? customParamsByIndex) {
  w.writeBool(customParamsByIndex == null); // quant_all_default
  if (customParamsByIndex != null) {
    for (var index = 0; index < 17; index++) {
      final DctParams? p = customParamsByIndex[index];
      if (p == null) {
      ...
      switch (p.mode) {
        ...
        case TransformMode.dct4:
          ...
          for (final List<double> ch in p.param!) {
...
const int _contextDomainSize = 495 * 15;
...
class _GroupTokens {
  /// Stores the contexts state used by the JPEG XL codec.
  ///
  final List<int> contexts = [];

  /// Stores the values state used by the JPEG XL codec.
  ///
...
(List<int>, int, int) _scanChannelValues(_TransformCtx ctx, Int32List acData) {
  final int numBlocks = ctx.numBlocks;
  final int ucoeffLen = ctx.orderSize - numBlocks;
  final int n = ctx.tt.pixelWidth;
  var countNonZero = 0;
  var lastNonZeroK = -1;
  final vals = List<int>.filled(ucoeffLen, 0);
  final bool flip = ctx.tt.flip;
  for (var k = 0; k < ucoeffLen; k++) {
    final int o = ctx.order[k + numBlocks];
    final int oy = o >> 16;
    ...
    final int v = flip ? acData[ox * n + oy] : acData[oy * n + ox];
    vals[k] = v;
    if (v != 0) {
      countNonZero++;
      lastNonZeroK = k;
...
/// Processes the block channel tokens data used by the JPEG XL codec.
///
int _blockChannelTokens(_TransformCtx ctx, HfBlockContext hfctx, int c, Int32List acData, int predicted, List<int> contextsOut, List<int> valuesOut) {
  final int numBlocks = ctx.numBlocks;
  final int orderSize = ctx.orderSize;
  final (List<int> vals, int countNonZero, int lastNonZeroK) = _scanChannelValues(ctx, acData);

  final int nonZeroCtx = HfCoefficients.getNonZeroContext(hfctx, predicted, ctx.blockCtx[c]);
  ...
  for (var k = 0; k <= lastNonZeroK; k++) {
    final prev = k == 0 ? (remaining > orderSize ~/ 16 ? 0 : 1) : (prevNonzero ? 1 : 0);
    final int coefCtx = ctx.histCtx[c] + HfCoefficients.getCoefficientContext(k + numBlocks, remaining, numBlocks, prev);
    final int u = _packSigned(vals[k]);
...
_GroupTokens _computeGroupTokens(
  ...
  for (final block in blocksInGroup) {
    final _TransformCtx ctx = ctxByType[block.tt.type]!;
    final int localY = block.by - groupOriginY;
    final int localX = block.bx - groupOriginX;
    final int numBlocks = ctx.numBlocks;
    final Int32List? predictedForBlock = predictedOut == null ? null : Int32List(3);
    for (final int c in _channelOrder) {
      final int predicted = HfCoefficients.getPredictedNonZeroes(nonZeroesGrid, c, localY, localX);
      predictedForBlock?[c] = predicted;
      final int countNonZero = _blockChannelTokens(ctx, hfctx, c, block.acInt[c], predicted, tokens.contexts, tokens.values);
      final int fill = (countNonZero + numBlocks - 1) ~/ numBlocks;
...
class _AcClustering {
  /// Stores the codes state used by the JPEG XL codec.
  ///
  final EntropyCodes codes;

  /// Stores the cluster map state used by the JPEG XL codec.
  ///
  final List<int> clusterMap;

  /// Stores the mapped clusters per group state used by the JPEG XL codec.
  ///
  final List<List<int>> mappedClustersPerGroup;

  /// Creates an internal ac clustering.
  ///
  _AcClustering({
    required this.codes,
...
_AcClustering _chooseAcClustering(List<_GroupTokens> groups) {
  ...
  for (final budget in candidateBudgets) {
    ...
    final codes = EntropyCodes.build(numContexts: numClustersUsed, contexts: allMapped, values: allValues, config: _hfConfig);
    ...
    if (best == null || bytes < totalBytes) {
      totalBytes = bytes;
      best = _AcClustering(codes: codes, clusterMap: fullClusterMap, mappedClustersPerGroup: mappedClustersPerGroup);
    }
  }
  if (const bool.fromEnvironment('jxl.encdebug')) {
    // ignore: avoid_print
...
double _blockRate(_TransformCtx ctx, HfBlockContext hfctx, List<Int32List> acIntCandidate, Int32List predictedPerChannel, List<int> clusterMap, List<List<int>> lengths) {
  var rate = 0.0;
  final contexts = <int>[];
  final values = <int>[];
  for (final int c in _channelOrder) {
    contexts.clear();
    values.clear();
    _blockChannelTokens(ctx, hfctx, c, acIntCandidate[c], predictedPerChannel[c], contexts, values);
...
double _tokenRate(List<int> clusterMap, List<List<int>> lengths, int contextId, int value) {
  ...
  final (int token, int extraBits, _) = tokenizeHybrid(_hfConfig, value);
...
/// Perceptual masking curve constants (see [VarDctConfiguration.perceptualMask]
...
/// [VarDctConfiguration.maskParamsOverride].
///
...
/// Stores the k mask knee state used internally by the JPEG XL codec.
///
const _kMaskKnee = 1.5;

/// Stores the k mask gamma state used internally by the JPEG XL codec.
///
const _kMaskGamma = 2.0;

/// Rate/distortion trade-off constant for the **masking** RD-hfMult path
/// (`_chooseHfMultRd` when [VarDctConfiguration.perceptualMask] is on), in
...
///
/// **Why [VarDctConfiguration.perceptualMask] still defaults off despite this being
...
/// sweep and analysis) — this is why [VarDctConfiguration.enableRdHfMult]
...
void _chooseHfMultRd(
  ...
  final bootstrapTokens = <_GroupTokens>[
    for (var gy = 0; gy < groupsY; gy++)
      for (var gx = 0; gx < groupsX; gx++) _computeGroupTokens(gy * 32, gx * 32, blocksByGroup[gy * groupsX + gx], hfctx, ctxByType, predictedOut: predictedOut),
  ];
  final _AcClustering bootstrap = _chooseAcClustering(bootstrapTokens);
  final List<List<int>> lengths = bootstrap.codes.tokenBitLengths();
  final List<int> clusterMap = bootstrap.clusterMap;
  ...
  final double kLambda = lambdaOverride ?? (perceptualMask ? _kMaskRdLambda : _kRdLambda);
  final ({double gamma, double hi, double knee})? maskParams = perceptualMask ? (maskOverride ?? (hi: _kMaskHi, knee: _kMaskKnee, gamma: _kMaskGamma)) : null;
  final double lambda = perceptualMask ? kLambda * acScale * acScale : kLambda * refStep * refStep;
  ...
  for (final block in placedBlocks) {
    final List<List<Float32List>> coeffBuf = block.computeCoeffBuf(planes, cfl, scratchA, scratchB);
    final _TransformCtx ctx = ctxByType[block.tt.type]!;
    ...
    if (maskParams != null) {
      ...
      if (spatialRel != null && block.tt.type == _tt8.type) {
        ...
        final TransformType tt = block.tt;
        final int ph = tt.pixelHeight;
        final int pw = tt.pixelWidth;
        final int llfH = tt.dctSelectHeight;
        final int llfW = tt.dctSelectWidth;
    ...
    for (final int mult in _rdHfMultCandidates) {
      ...
      final double rate = _blockRate(ctx, hfctx, candidate.ac, predicted, clusterMap, lengths);
...
void _rdoqBlockChannel(
  ...
  final int n = ctx.tt.pixelWidth;
  final bool flip = ctx.tt.flip;
  final Int32List acData = block.acInt[c];
  final (List<int> vals, int countNonZero, int lastNonZeroK) = _scanChannelValues(ctx, acData);
  if (countNonZero == 0) {
    return;
  }
  ...
  final int nonZeroCtxId = HfCoefficients.getNonZeroContext(hfctx, predicted, ctx.blockCtx[c]);
  final int histCtx = ctx.histCtx[c];
  ...
  double rateAt(int k, int remainingForCtx, int packedValue) {
    ...
    final int coefCtx = histCtx + HfCoefficients.getCoefficientContext(k + numBlocks, remainingForCtx, numBlocks, prev);
...
void _chooseAcRdoq(
  ...
  final List<List<int>> lengths = bootstrap.codes.tokenBitLengths();
  final List<int> clusterMap = bootstrap.clusterMap;
  final double lambda = (lambdaOverride ?? _kRdoqLambda) * acScale * acScale;

  for (final block in placedBlocks) {
    if (block.hfMult == _rdoqExemptHfMult) {
      continue;
    }
    final List<List<Float32List>> coeffBuf = block.computeCoeffBuf(planes, cfl, scratchA, scratchB);
    final _TransformCtx ctx = ctxByType[block.tt.type]!;
    ...
    final Int32List predicted = predictedOut[block]!;
    for (final int c in _channelOrder) {
      _rdoqBlockChannel(ctx, hfctx, c, coeffBuf[c], rawWeight[c], scaleFactor[c] / block.hfMult, predicted[c], clusterMap, lengths, lambda, block);
    }
  }
}
