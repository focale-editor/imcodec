...
import 'package:imcodec/src/codecs/jpeg_xl/encode/var_dct/var_dct_encoder.dart';
...
/// Processes the hist entropy data used by the JPEG XL codec.
///
double _histEntropy(Int32List hist, int off, int width, int total) {
  ...
  for (var t = 0; t < width; t++) {
    final int c = hist[off + t];
    if (c > 0) {
      bits += c * (math.log(lt / c) * 1.4426950408889634);
    }
  }
  return bits;
}
...
List<int> _selectLeafPredictors(
  ...
  for (var c = 0; c < numContexts; c++) {
    ...
    final double pBits = _histEntropy(pHist, c * w, w, total) + pExtra[c];
    final double aBits = _histEntropy(aHist, c * w, w, total) + aExtra[c];
...
Uint8List _encodeModularCore(JpegXlEncodeSetup setup, List<Int32List> inputPlanes, List<int>? palette, bool applyRct, {int paletteChannels = 3}) {
  ...
  final int groupsX = ceilDiv(width, groupDim);
  final int groupsY = ceilDiv(height, groupDim);
  final int numGroups = groupsX * groupsY;
  final int numLfGroups = ceilDiv(width, groupDim << 3) * ceilDiv(height, groupDim << 3);
  final singleSection = numGroups == 1;
  final bool globalChannels = width <= groupDim && height <= groupDim;
  ...
  _Prep prep(bool useWp) {
    ...
    final List<List<int>>? groupMaxErr = useWp ? List<List<int>>.generate(numGroups, (_) => []) : null;
    for (var g = 0; g < numGroups; g++) {
      final int ox = (g % groupsX) * groupDim;
      final int oy = (g ~/ groupsX) * groupDim;
      final int tw = (width - ox).clamp(0, groupDim);
      final int th = (height - oy).clamp(0, groupDim);
  ...
  (Uint8List, String, int, bool, bool, bool) assembleStream(
    ...
    final int numContexts = tree.contexts;
    ...
    late final List<Lz77Ops> sectionOpsShallow = [for (var s = 0; s < sectionValues.length; s++) lz77Compress(sectionContexts[s], sectionValues[s])];
    late final List<Lz77Ops> sectionOpsDeep = [for (var s = 0; s < sectionValues.length; s++) lz77Compress(sectionContexts[s], sectionValues[s], effort: Lz77Effort.deep)];

    Uint8List assemble(EntropyCodes codes, bool ans, bool lz, List<Lz77Ops> ops) {
      ...
      if (palette != null) {
        ...
        lfGlobal.writeU32(palette.length, 0, 8, 256, 10, 1280, 12, 5376, 16); // nb_colors
      ...
      if (singleSection) {
        final Uint8List body = lfGlobal.toBytes();
        ...
        final sections = <Uint8List>[lfGlobal.toBytes(), for (var i = 0; i < numLfGroups; i++) Uint8List(0), Uint8List(0), for (var g = 0; g < numGroups; g++) writeGroupSection(1 + g)];
    ...
    if (only != null) {
      ...
      final codes = lz
          ? EntropyCodes.buildLz77(numContexts: numContexts, sections: ops, config: cfg)
          : EntropyCodes.build(numContexts: numContexts, contexts: flatContexts(), values: flatValues(), config: cfg);
    ...
    for (var ci = 0; ci < _hybridConfigs.length; ci++) {
      ...
      final lzCodes = EntropyCodes.buildLz77(numContexts: numContexts, sections: sectionOpsShallow, config: cfg);
      final plainCodes = EntropyCodes.build(numContexts: numContexts, contexts: allContexts, values: allValues, config: cfg);
    ...
    if (bestShallowLz < bestPlain) {
      for (var ci = 0; ci < _hybridConfigs.length; ci++) {
        final HybridIntegerConfig cfg = _hybridConfigs[ci];
        final lzCodes = EntropyCodes.buildLz77(numContexts: numContexts, sections: sectionOpsDeep, config: cfg);
  ...
  (Uint8List, String) refineOf(_Prep p, _Prep alt, _PassB ctx, Uint8List baseBytes, String baseLabel, (int, bool, bool, bool) baseSel) {
    ...
    final List<List<int>> mixedGroups = [for (var g = 0; g < numGroups; g++) _mixValues(ctx.groupContexts[g], p.groupValues[g], alt.groupValues[g], leafPred, p.predictor)];
