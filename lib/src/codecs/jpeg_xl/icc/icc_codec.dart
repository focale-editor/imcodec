...
abstract final class IccCodec {
  /// Stores the mntr rgb state used by the JPEG XL codec.
  ///
  static const _mntrRgb = 'mntrRGB XYZ ';

  /// Stores the acsp state used by the JPEG XL codec.
  ///
  static const _acsp = 'acsp';

  /// Stores the icc tags state used by the JPEG XL codec.
  ///
  ...
  /// Stores the sized20 tags state used by the JPEG XL codec.
  ///
  ...
  static Uint8List readEncodedStream(BitReader reader, int encodedSize) {
    final stream = EntropyStream.read(reader: reader, numDists: 41);
  ...
  /// Processes prediction data for the JPEG XL codec.
  ///
  static int _prediction(Uint8List buffer, int outputSize, int i) {
    if (i <= 3) {
      return outputSize >> (8 * (3 - i)) & 0xFF;
    }
    if (i == 8) {
      return 4;
    }
    if (i >= 12 && i <= 23) {
      return _mntrRgb.codeUnitAt(i - 12);
    }
    if (i >= 36 && i <= 39) {
      return _acsp.codeUnitAt(i - 36);
    }
    if (buffer[40] == 0x41 /* A */ ) {
      if (i == 41 || i == 42) {
        return 0x50; // P
  ...
  /// Processes shuffle data for the JPEG XL codec.
  ///
  ...
  static Uint8List decompress(Uint8List encoded) {
    ...
    for (var i = 0; i < headerSize; i++) {
      ...
      final int p = _prediction(result, outputSize, i);
      put(p + e);
    }
    if (pos == outputSize) {
      return result;
