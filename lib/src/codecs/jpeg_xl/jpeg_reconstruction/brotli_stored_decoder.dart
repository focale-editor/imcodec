...
/// Throws [JpegXlUnsupportedException] (`'brotli-compressed'`) on the first
/// genuinely compressed meta-block; a full RFC 7932 decoder is a later phase.
Uint8List decodeStoredBrotliStream(BitReader br) {
  ...
  while (true) {
    ...
    if (isLast && br.readBits(1) == 1) {
      break; // ISLASTEMPTY -> stream ends.
    }

    final int mnibCode = br.readBits(2);
    if (mnibCode == 3) {
      // Metadata meta-block: no uncompressed output; skip its bytes.
      br.readBits(1); // reserved (must be 0)
      final int mskipBytes = br.readBits(2);
      final int mskipLen = mskipBytes > 0 ? br.readBits(mskipBytes * 8) + 1 : 0;
      _alignToByte(br);
    ...
    final int mlen = br.readBits(numNibbles * 4) + 1;
    ...
    if (isLast || br.readBits(1) != 1) {
      throw JpegXlUnsupportedException(feature: 'brotli-compressed');
    }

    _alignToByte(br);
    final Uint8List chunk = br.remainingBytes();
    if (chunk.length < mlen) {
      throw const JpegXlTruncatedException(message: 'truncated brotli stored block');
    }
