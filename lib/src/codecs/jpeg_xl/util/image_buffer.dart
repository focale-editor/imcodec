...
void _checkPlaneSize(int height, int width) {
  if (height < 0 || width < 0 || (width != 0 && height > JpegXlLimits.maxPlanePixels ~/ width)) {
    throw JpegXlInvalidBitstreamException(message: 'plane ${width}x$height exceeds JpegXlLimits.maxPlanePixels');
  }
}

/// Represents Image buffer data used while processing JPEG XL images.
///
final class ImageBuffer {
  ...
  /// Stores the int rows state used internally by the JPEG XL codec.
  ///
  List<Int32List>? _intRows;

  /// Stores the float rows state used internally by the JPEG XL codec.
  ///
  List<Float32List>? _floatRows;

  /// Processes int32 information in a JPEG XL codestream.
  ///
  factory ImageBuffer.int32({
    required int height,
    required int width,
  }) {
    _checkPlaneSize(height, width);
    return ImageBuffer._int32(height: height, width: width);
  }

  /// Creates Image buffer state for JPEG XL processing.
  ///
  ImageBuffer._int32({
    required this.height,
    required this.width,
  }) : _intRows = List.generate(height, (_) => Int32List(width), growable: false),
       _floatRows = null;

  /// Processes float32 information in a JPEG XL codestream.
  ///
  factory ImageBuffer.float32({
    required int height,
    required int width,
  }) {
    _checkPlaneSize(height, width);
    return ImageBuffer._float32(height: height, width: width);
  }

  /// Creates Image buffer state for JPEG XL processing.
  ///
  ImageBuffer._float32({
    required this.height,
    required this.width,
  }) : _intRows = null,
       _floatRows = List.generate(height, (_) => Float32List(width), growable: false);

  /// Processes copy information in a JPEG XL codestream.
  ///
  ImageBuffer.copy({
    required ImageBuffer other,
    bool copyData = true,
  }) : height = other.height,
       width = other.width,
       _intRows = switch (other._intRows) {
         null => null,
         final List<Int32List> rows => List.generate(other.height, (y) => copyData ? Int32List.fromList(rows[y]) : Int32List(other.width), growable: false),
       },
       _floatRows = switch (other._floatRows) {
         null => null,
         final List<Float32List> rows => List.generate(other.height, (y) => copyData ? Float32List.fromList(rows[y]) : Float32List(other.width), growable: false),
       };

  /// Stores the is int value used while processing JPEG XL data.
  ///
  bool get isInt => _intRows != null;

  /// Stores the is float value used while processing JPEG XL data.
  ///
  bool get isFloat => _floatRows != null;

  /// Stores the int rows value used while processing JPEG XL data.
  ///
  List<Int32List> get intRows => _intRows!;

  /// Stores the float rows value used while processing JPEG XL data.
  ///
  ...
  void castToFloatWithMax(int maxValue) {
    final List<Int32List>? ints = _intRows;
    if (ints == null) {
      return;
    }
    assert(maxValue >= 1, 'The maximum sample value must be positive.');
    final double scale = 1.0 / maxValue;
    final List<Float32List> floats = List.generate(height, (y) {
      final Int32List src = ints[y];
  ...
  /// Processes cast to float information in a JPEG XL codestream.
  ///
  ...
  void castToIntWithMax(int maxValue) {
    final List<Float32List>? floats = _floatRows;
    if (floats == null) {
      return;
    }
    assert(maxValue >= 1, 'The maximum sample value must be positive.');
    final List<Int32List> ints = List.generate(height, (y) {
      final Float32List src = floats[y];
      final row = Int32List(width);
      for (var x = 0; x < width; x++) {
        final int v = (src[x] * maxValue + 0.5).truncate();
        row[x] = v < 0
            ? 0
            : v > maxValue
            ? maxValue
  ...
  /// Processes cast to int information in a JPEG XL codestream.
  ///
  ...
  void reconstructFloatSamples(int bits, int expBits) {
    ...
    final List<Float32List> floats = List.generate(height, (_) => Float32List(width), growable: false);
    ...
    if (bits == 32) {
      ...
      for (var y = 0; y < height; y++) {
        final Int32List src = ints[y];
        final Float32List dst = floats[y];
        for (var x = 0; x < width; x++) {
    ...
    final int expBias = (1 << (expBits - 1)) - 1;
    final int signShift = bits - 1;
    final int mantBits = bits - expBits - 1;
    final int mantShift = 23 - mantBits;
    final int expAllOnes = (1 << expBits) - 1;

    for (var y = 0; y < height; y++) {
      final Int32List src = ints[y];
      final Float32List dst = floats[y];
      for (var x = 0; x < width; x++) {
        final int raw = src[x] & 0xFFFFFFFF;
        final signbit = (raw >> signShift) != 0;
        final int f = raw & ((1 << signShift) - 1);
        ...
        exp = exp - expBias + 127;
        assert(exp >= 0, 'The reconstructed exponent cannot be negative.');
...
List<Float32x4List> rowVectorViews(List<Float32List> rows) =>
    List<Float32x4List>.generate(rows.length, (y) => Float32x4List.view(rows[y].buffer, rows[y].offsetInBytes, rows[y].length >> 2), growable: false);

/// Processes float matrix information in a JPEG XL codestream.
///
List<Float32List> floatMatrix(int height, int width) => List.generate(height, (_) => Float32List(width), growable: false);
