...
int unpackSigned(int value) => (value & 1) == 0 ? value >> 1 : -(value >> 1) - 1;

/// Returns the ceiling of an integer division.
@pragma('vm:prefer-inline')
int ceilDiv(int numerator, int denominator) => (numerator + denominator - 1) ~/ denominator;
...
int floorLog1p(int x) {
  final int c = x.bitLength;
  return (x + 1) & x != 0 ? c - 1 : c;
}

/// Returns the real cube root of [value].
double realCubeRoot(double value) => value < 0 ? -math.pow(-value, 1 / 3).toDouble() : math.pow(value, 1 / 3).toDouble();

/// Returns two raised to any non-negative [exponent].
/// This is computed without ever needing a `<<`
...
@pragma('vm:prefer-inline')
int _integerPowerOfTwo(int exponent) => exponent < 32 ? (1 << exponent) : (1 << (exponent - 32)) * 0x100000000;

/// Left-shifts non-negative [value] by [shiftCount] bits.
/// The shift count may be at least 32, and
/// [value] itself may already need more than 32 bits) - safe on dart2js
/// where a bare `<<` is not. See [_integerPowerOfTwo]. Results are only exact while the
...
/// platform.
@pragma('vm:prefer-inline')
int wideShl(int value, int shiftCount) => value * _integerPowerOfTwo(shiftCount);

/// Right-shifts non-negative [value] by [shiftCount] bits.
/// This is the division-based
/// counterpart to [wideShl] for the same reason (also safe when [value]
/// itself needs more than 32 bits, unlike a bare `>>`).
@pragma('vm:prefer-inline')
int wideShr(int value, int shiftCount) => value ~/ _integerPowerOfTwo(shiftCount);

/// Arithmetic right-shift of [value] (which may be negative, unlike
/// [wideShr]) by [shiftCount] bits.
/// This matches `>>`'s floor-toward-negative-infinity
...
@pragma('vm:prefer-inline')
int wideShrSigned(int value, int shiftCount) {
  final int divisor = _integerPowerOfTwo(shiftCount);
  final int quotient = value ~/ divisor;
  return value < 0 && quotient * divisor != value ? quotient - 1 : quotient;
}

/// Reflects an out-of-range coordinate back into [0, size).
@pragma('vm:prefer-inline')
int mirrorCoordinate(int originalCoordinate, int size) {
  int coordinate = originalCoordinate;
  while (coordinate < 0 || coordinate >= size) {
    final int tc = ~coordinate;
...
double erf(double z) {
  final double az = z.abs();
  double absErf;
  if (az > 1e-4) {
    final double t = 1.0 / (az * 0.5 + 1.0);
    final double u =
        t * (t * (t * (t * (t * (t * (t * (t * (t * 0.17087277 - 0.82215223) + 1.48851587) - 1.13520398) + 0.27886807) - 0.18628806) + 0.09678418) + 0.37409196) + 1.00002368) - 1.26551223;
    absErf = 1.0 - t * math.exp(-z * z + u);
  } else {
    final double t = 1.0 / (az * 0.47047 + 1.0);
    final double u = t * (t * (t * 0.7478556 - 0.0958798) + 0.3480242);
