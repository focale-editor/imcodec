...
final class JpegCoeffSink {
  ...
  /// Stores the coeffs value used while processing JPEG XL data.
  ///
  ...
  /// Creates Jpeg coeff sink data for JPEG XL processing.
  ///
  JpegCoeffSink({
    required this.widthInBlocks,
    required this.heightInBlocks,
  }) : coeffs = [for (var i = 0; i < widthInBlocks.length; i++) Int32List(widthInBlocks[i] * heightInBlocks[i] * 64)],
       cflFactor = [for (var i = 0; i < widthInBlocks.length; i++) Int32List(widthInBlocks[i] * heightInBlocks[i])];

  /// Records the per-tile CfL factor for one block.
  void setFactor(int component, int blockY, int blockX, int value) {
    if (component >= coeffs.length) {
      return;
    }
    final int wib = widthInBlocks[component];
    if (blockX >= wib || blockY >= heightInBlocks[component]) {
      return;
    }
  ...
  void setAcBlock(int component, int blockY, int blockX, Float32List qc, int pixelY, int pixelX, int qw) {
    if (component >= coeffs.length) {
      return;
    }
    final int wib = widthInBlocks[component];
    ...
    final int dst = (blockY * wib + blockX) * 64;
    final Int32List out = coeffs[component];
    for (var r = 0; r < 8; r++) {
      final int srcRow = (pixelY + r) * qw + pixelX;
      final int dstRow = dst + r * 8;
      for (var col = 0; col < 8; col++) {
        if (r == 0 && col == 0) {
          continue; // DC comes from setDc.
        }
