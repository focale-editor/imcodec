...
/// transform, converting the codec's linear sRGB-primaries RGB (the state right
...
final class IccRgbOutputTransform {
  ...
  static IccRgbOutputTransform? tryParse(Uint8List icc) {
    ...
    int be32(int o) => (icc[o] << 24) | (icc[o + 1] << 16) | (icc[o + 2] << 8) | icc[o + 3];
    ...
    final int tagCount = be32(128);
    // Bound the tag table against the profile length (robustness contract).
    if (tagCount < 0 || 132 + tagCount * 12 > icc.length) {
      return null;
    }
    final tags = <String, (int off, int len)>{};
    for (var i = 0; i < tagCount; i++) {
      final int e = 132 + i * 12;
      final int off = be32(e + 4);
      final int len = be32(e + 8);
      if (off < 0 || len < 0 || off + len > icc.length) {
        return null;
      }
      tags[sig(e)] = (off, len);
    }
    ...
    if (tags.containsKey('A2B0') || tags.containsKey('mAB ')) {
      return null;
    }
    ...
    final List<double> r = colorant('rXYZ');
    final List<double> g = colorant('gXYZ');
    final List<double> b = colorant('bXYZ');
    final List<List<double>> prof = [
      [r[0], g[0], b[0]],
      [r[1], g[1], b[1]],
      [r[2], g[2], b[2]],
    ];
  ...
  static Float32List? _parseInverseTrc(Uint8List icc, int off, int Function(int) be32, double Function(int) fix16, String Function(int) sig) {
    final String type = sig(off);
    double Function(double) dev2lin;
    if (type == 'curv') {
      final int count = be32(off + 8);
      if (count < 0 || off + 12 + count * 2 > icc.length) {
        return null;
      }
      if (count == 0) {
        dev2lin = (x) => x; // identity
      } else if (count == 1) {
        final double g = ((icc[off + 12] << 8) | icc[off + 13]) / 256.0; // u8Fixed8
        dev2lin = (x) => math.pow(x < 0 ? 0 : x, g).toDouble();
      } else {
        final lut = Float64List(count);
        for (var k = 0; k < count; k++) {
          lut[k] = ((icc[off + 12 + k * 2] << 8) | icc[off + 13 + k * 2]) / 65535.0;
        }
        dev2lin = (x) {
          if (x <= 0) {
            return lut[0];
          ...
          final double f = x * (count - 1);
          final int i = f.toInt();
          return lut[i] + (lut[i + 1] - lut[i]) * (f - i);
        };
      }
    } else if (type == 'para') {
      final int ft = (icc[off + 8] << 8) | icc[off + 9];
      final int? np = const {0: 1, 1: 3, 2: 4, 3: 5, 4: 7}[ft];
      if (np == null || off + 12 + np * 4 > icc.length) {
        return null;
      }
      final List<double> p = [for (var k = 0; k < np; k++) fix16(off + 12 + k * 4)];
      final double g = p[0];
    ...
    for (var k = 0; k <= lutSize; k++) {
      final double targetLin = k / lutSize;
      var lo = 0.0;
      var hi = 1.0;
