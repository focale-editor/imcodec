...
final class BmpDecoder {
  ...
  Image decode(Uint8List bytes, {required int maxPixels}) {
    ...
    if (bitsPerPixel <= 8) {
      ...
      for (int index = 0; index < paletteLength; index++) {
        ...
        palette.add(_Rgba(red, green, blue, 255));
      }
    }
  ...
  _Rgba _readPixel(
    ...
    if (bitsPerPixel == 24) {
      ...
      return _Rgba(bytes[offset + 2], bytes[offset + 1], bytes[offset], 255);
    }
    ...
    return _Rgba(_scaleMask(value, redMask), _scaleMask(value, greenMask), _scaleMask(value, blueMask), alpha);
  }
