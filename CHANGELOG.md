# 📰 Imcodec changelog

## v0.3.1
Released on August 29, 2026.

* **FEAT**: Added static GIF encoding and first-frame GIF decoding. ([#756a7f6](https://github.com/focale-editor/imcodec/commit/756a7f6))

## v0.3.0
Released on August 28, 2026.

* **BREAKING REFACTOR**: Updated `zcodec`. ([#4d80530](https://github.com/focale-editor/imcodec/commit/4d80530))

## v0.2.0
Released on August 27, 2026.

* **FEAT**: Added a lossy WebP encoder. ([#1755f14](https://github.com/focale-editor/imcodec/commit/1755f14))
* **FEAT**: Added parallel encoding support via `ParallelRunner` for JPEG, JPEG XL, PNG, and WebP. ([#a2d71ff](https://github.com/focale-editor/imcodec/commit/a2d71ff))
* **FIX**: Fixed exports of the root library. ([#74383db](https://github.com/focale-editor/imcodec/commit/74383db))
* **CHORE**: Improved encoding and decoding speed for JPEG XL. ([#5777ffc](https://github.com/focale-editor/imcodec/commit/5777ffc))
* **CHORE**: Improved encoding and decoding speed for various formats. ([#576d4bb](https://github.com/focale-editor/imcodec/commit/576d4bb))
* **CHORE**: Improved the WebP encoder. ([#63ef3d5](https://github.com/focale-editor/imcodec/commit/63ef3d5))
* **BREAKING REFACTOR**: Refactored the codecs behavior by adding a true `Codec`, a decoder and an encoder. ([#93eae02](https://github.com/focale-editor/imcodec/commit/93eae02))
* **REFACTOR**: Improved JPEG and WEBP codecs coding style. ([#3c4dcc0](https://github.com/focale-editor/imcodec/commit/3c4dcc0))

## v0.1.5
Released on August 25, 2026.

* **FEAT**: Added pure-Dart JPEG XL import for Modular and VarDCT codestreams and containers, plus lossless Modular RGBA export. ([#84d9de6](https://github.com/focale-editor/imcodec/commit/84d9de6))

## v0.1.4
Released on August 25, 2026.

* **CHORE**: Added `build` to `.pubignore`. ([#6eba251](https://github.com/focale-editor/imcodec/commit/6eba251))
* **CHORE**: Ignored some files for publishing on pub.dev. ([#d1ce6aa](https://github.com/focale-editor/imcodec/commit/d1ce6aa))

## v0.1.3
Released on August 24, 2026.

* **CHORE**: Added an example. ([#dbec106](https://github.com/focale-editor/imcodec/commit/dbec106))
* **CHORE**: Updated `zcodec` constraint. ([#92855f6](https://github.com/focale-editor/imcodec/commit/92855f6))

## v0.1.2
Released on August 23, 2026.

* **CHORE**: Updated README. ([#fb295e1](https://github.com/focale-editor/imcodec/commit/fb295e1))

## v0.1.1
Released on August 23, 2026.

* **CHORE**: Updated dependencies. ([#8e0c0b0](https://github.com/focale-editor/imcodec/commit/8e0c0b0))

## v0.1.0
Released on August 22, 2026.

* **Initial release.**
