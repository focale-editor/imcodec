# Contributing to Imcodec

Thanks for helping improve Imcodec. Bug reports, focused fixes, documentation improvements, and well-scoped features are welcome.
...
- Search [existing issues](https://github.com/focale-editor/imcodec/issues) before opening a new one.
...
Imcodec follows the SDK constraint in [`pubspec.yaml`](pubspec.yaml). Install a compatible stable Flutter SDK, fork the repository, and run:

```sh
git clone https://github.com/YOUR_ACCOUNT/imcodec.git
cd imcodec
flutter pub get
```
...
Codec changes should include focused round-trip, malformed-input, and limit tests where applicable.
